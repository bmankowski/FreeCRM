...
class m250101_000001_create_import_tables extends Migration
{
	public function safeUp(): void
	{
		...
		$this->createIndex('idx_import_batches_status', '{{%import_batches}}', ['status']);
		$this->createIndex('idx_import_batches_module', '{{%import_batches}}', ['module']);
		$this->createIndex('idx_import_batches_cleanup', '{{%import_batches}}', ['cleanup_after']);

		$this->createTable('{{%import_mappings}}', [
			'id' => $this->primaryKey(),
		...
		$this->createIndex('idx_import_mappings_batch', '{{%import_mappings}}', ['batch_id']);

		$this->createTable('{{%import_logs}}', [
			'id' => $this->primaryKey(),
			'batch_id' => $this->integer()->notNull(),
		...
		$this->createIndex('idx_import_logs_batch', '{{%import_logs}}', ['batch_id']);
		$this->createIndex('idx_import_logs_status', '{{%import_logs}}', ['status']);
	...
	{
		$this->dropForeignKey('fk_import_logs_batch', '{{%import_logs}}');
		$this->dropForeignKey('fk_import_mappings_batch', '{{%import_mappings}}');

		$this->dropTable('{{%import_logs}}');
		$this->dropTable('{{%import_mappings}}');
