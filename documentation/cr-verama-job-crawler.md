...
Dodać **dedykowany crawler Verama** (Ework Verama), który:
...
- **Jedno źródło w tym CR:** tylko Verama. Kod w `scripts/job-crawlers/verama/` — bez wspólnego frameworka multi-source; katalog nadrzędny zostawia miejsce na kolejne crawler’y.
...
|------|------|-------------|
| `scripts/job-crawlers/verama/` | Pakiet crawlera (CLI, client, parser, writer) | CLI / pliki |
| `scripts/job-crawlers/verama/requirements.txt` | `requests`, `beautifulsoup4`, `lxml` (parser BS4) | Build |
| `scripts/job-crawlers/verama/README.md` | Jak uruchomić, schema JSON, sekrety | Docs |
| `docker/verama-crawler/Dockerfile` | Obraz Python | Docker |
| `docker/verama-crawler/verama.env.example` | Szablon credentials (bez haseł) | Config |
| `docker-compose.yml` | Serwis `verama-crawler` | Dev ops |
| `.gitignore` | `docker/verama-crawler/verama.env` | Secrets |
| `documentation/cr-verama-job-crawler.md` | Ten CR | Docs |
...
| `.gitignore` | Ignore `verama.env` | Internal |
...
| F8 | Raport końcowy (stdout + `cache/logs/verama-crawler.log`): fetched / written / closed / failed / duration. |
| F9 | Run ręczny: `docker compose run --rm verama-crawler`. |
...
flowchart TB
  ...
  subgraph crawler [verama-crawler container]
    CLI[cli.py]
    Auth[auth]
    Client[api client]
  ...
  subgraph fs [import/jobs]
    ...
    Failed[failed/]
    Log[cache/logs/verama-crawler.log]
  end
  CLI --> Auth --> Login
  Auth --> Client
...
docker/verama-crawler/
  Dockerfile
  verama.env.example
  verama.env           # gitignored — local only
```
...
| Variable | Where |
|----------|--------|
| `VERAMA_EMAIL` | `docker/verama-crawler/verama.env` |
| `VERAMA_PASSWORD` | same |
| Optional overrides | `VERAMA_BASE_URL`, `VERAMA_REQUEST_DELAY_SEC=2`, `VERAMA_OUTPUT_DIR=/var/www/import/jobs` |

Compose: `env_file: docker/verama-crawler/verama.env`.  
**Nie** commitować `verama.env`. Hasło podane w czacie projektowym — **zmienić w Veramie** przed użyciem w pliku lokalnym.
...
verama-crawler:
  build:
    context: .
    dockerfile: docker/verama-crawler/Dockerfile
  env_file:
    - docker/verama-crawler/verama.env
  volumes:
    - ./import:/var/www/import
    - ./cache/logs:/var/www/html/cache/logs
    - ./scripts/job-crawlers/verama:/app:ro
  working_dir: /app
  profiles: ["crawler"]   # nie startuje z domyślnym `up`
  restart: "no"
```
...
```bash
docker compose --profile crawler run --rm verama-crawler
```
...
1. **Scaffold** — `scripts/job-crawlers/verama/` + `requirements.txt` + README stub; `.gitignore` dla `verama.env`; `verama.env.example`.
...
6. **CLI + report + logging** — stdout + `cache/logs/verama-crawler.log`.
7. **Docker** — Dockerfile, compose service z profile `crawler`, dokumentacja run.
...
1. Skopiuj `verama.env.example` → `verama.env`, uzupełnij credentials (po rotacji hasła).
2. `docker compose --profile crawler build verama-crawler`
3. `docker compose --profile crawler run --rm verama-crawler`
...
- `cache/logs/verama-crawler.log`
- stdout kontenera

### Automated tests

- Lekkie unit testy parsera `description.py` na 2–3 zapisanych fragmentach HTML z Veramy (fixture w `scripts/job-crawlers/verama/tests/`) — zalecane w tym CR.
...
- Usunąć serwis z compose / nie budować obrazu; usunąć katalog `scripts/job-crawlers/verama/` i `docker/verama-crawler/`.
...
2. **Cron 10 min** w `verama-crawler` / supercronic.
