...
class TextParser
{
	...
	protected function translate($params)
	{
		if (strpos($params, '|') === false) {
			return \App\Runtime\Vtiger_Language_Handler::translate($params);
		}
		...
		switch ($key) {
			...
			case 'ChangesListChanges':
				foreach ($this->recordModel->getPreviousValue() as $fieldName => $oldValue) {
					...
					if ($this->withoutTranslations !== true) {
						$value .= \App\Runtime\Vtiger_Language_Handler::translate($fieldModel->getFieldLabel(), $this->moduleName, $this->language) . ' ';
						$value .= \App\Runtime\Vtiger_Language_Handler::translate('LBL_FROM') . " $oldValue " . \App\Runtime\Vtiger_Language_Handler::translate('LBL_TO') . " $currentValue" . PHP_EOL;
					} else {
			...
			case 'ChangesListValues':
				foreach ($this->recordModel->getPreviousValue() as $fieldName => $oldValue) {
					...
					$currentValue = $this->recordDisplayValue($this->recordModel->get($fieldName), $fieldModel);
					if ($this->withoutTranslations !== true) {
						$value .= \App\Runtime\Vtiger_Language_Handler::translate($fieldModel->getFieldLabel(), $this->moduleName, $this->language) . ": $currentValue" . PHP_EOL;
					} else {
	...
	public function getRelatedVariable($fieldType = false)
	{
		...
		foreach ($moduleModel->getFieldsByType(array_merge(\App\Modules\Vtiger\Models\Field::$referenceTypes, ['owner', 'multireference'])) as $parentFieldName => $field) {
			...
			$parentFieldNameLabel = \App\Runtime\Vtiger_Language_Handler::translate($field->getFieldLabel(), $this->moduleName);
			if (!$fieldType) {
				foreach (static::$variableEntity as $key => $name) {
					$variables[$parentFieldName]["$parentFieldNameLabel - $entityVariables"][] = [
						...
						'label' => $parentFieldNameLabel . ': ' . \App\Runtime\Vtiger_Language_Handler::translate($name)
					];
				}
			}
			foreach ($relatedModules as $relatedModule) {
				...
				foreach ($moduleModel->getBlocks() as $blockModel) {
					foreach ($blockModel->getFields() as $fieldName => $fieldModel) {
						if ($fieldModel->isViewable() && !($fieldType && $fieldModel->getFieldDataType() !== $fieldType)) {
							$labelGroup = "$parentFieldNameLabel: ($relatedModuleLang) " . \App\Runtime\Vtiger_Language_Handler::translate($blockModel->get('label'), $relatedModule);
							$variables[$parentFieldName][$labelGroup][] = [
								...
								'label' => "$parentFieldNameLabel: ($relatedModuleLang) " . \App\Runtime\Vtiger_Language_Handler::translate($fieldModel->getFieldLabel(), $relatedModule)
							];
						}
					}
				}
	...
	public function getGeneralVariable()
	{
		...
		foreach (array_keys($companyDetails) as $name) {
			$companyVariables["$(organization : $name)$"] = \App\Runtime\Vtiger_Language_Handler::translate('LBL_' . strtoupper($name), 'Settings:Companies');
		}
		$companyVariables['$(organization : mailLogo)$'] = \App\Runtime\Vtiger_Language_Handler::translate('LBL_LOGO_IMG_MAIL', 'Settings:Companies');
		$companyVariables['$(organization : loginLogo)$'] = \App\Runtime\Vtiger_Language_Handler::translate('LBL_LOGO_IMG_LOGIN', 'Settings:Companies');
		$companyVariables['$(organization : logo_login)$'] = \App\Runtime\Vtiger_Language_Handler::translate('LBL_LOGO_PATH_LOGIN', 'Settings:Companies');
		$companyVariables['$(organization : logo_main)$'] = \App\Runtime\Vtiger_Language_Handler::translate('LBL_LOGO_PATH_MAIN', 'Settings:Companies');
		$companyVariables['$(organization : logo_mail)$'] = \App\Runtime\Vtiger_Language_Handler::translate('LBL_LOGO_PATH_MAIL', 'Settings:Companies');
		...
		if ($this->moduleName && is_dir(("modules/{$this->moduleName}/textParsers/"))) {
			foreach ((new \DirectoryIterator("modules/{$this->moduleName}/textParsers/")) as $fileInfo) {
				...
				if ($fileInfo->getType() !== 'dir' && $fileInfo->getExtension() === 'php') {
					...
					$variables["$(custom : $fileName|{$this->moduleName})$"] = \App\Runtime\Vtiger_Language_Handler::translate($instanceClass->name, $this->moduleName);
				}
			}
		}
		return $variables;
