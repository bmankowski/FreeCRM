...
class VTScheduledReport extends Reports
{
		...
		if (!empty($this->id)) {
			...
			if ($cachedInfo === false) {
				...
				if ($adb->num_rows($result) > 0) {
					...
					$scheduledInterval = (!empty($reportScheduleInfo['schedule'])) ? \App\Utils\Json::decode($reportScheduleInfo['schedule']) : array();
					$scheduledRecipients = (!empty($reportScheduleInfo['recipients'])) ? \App\Utils\Json::decode($reportScheduleInfo['recipients']) : array();
		...
		if (!empty($recipientsInfo)) {
			...
			if (!empty($recipientsInfo['rs'])) {
				foreach ($recipientsInfo['rs'] as $roleId) {
					$users = \App\Security\PrivilegeUtil::getRoleAndSubordinateUsers($roleId);
	...
	public function sendEmail()
	{
		...
		$subject = $this->reportname . ' - ' . $currentTime . ' (' . \App\Fields\DateTimeField::getDBTimeZone() . ')';
	...
	public static function generateRecipientOption($type, $value, $name = '')
	{
		switch ($type) {
			...
			case 'roles' : if (empty($name))
					$name = \App\Security\PrivilegeUtil::getRoleName($value);
	...
	{
		$roleDetails = \App\Security\PrivilegeUtil::getAllRoleDetails();
