...
class InitSchema_Model
{
	...
	public function initialize()
	{
		...
		$this->db->pquery('UPDATE vtiger_version SET `current_version` = ?, `old_version` = ? ;', [\App\Core\Version::get(), \App\Core\Version::get()]);
	...
	public function deleteDirFile($src)
	{
		...
		if (is_dir($src)) {
			...
			foreach ($dir as $fileinfo) {
				if (!$fileinfo->isDot()) {
					if ($fileinfo->isDir()) {
						\App\Log\Log::trace("Exiting VT620_to_YT::testest 22(" . $fileinfo->getPathname() . ") method ...");
	 ...
	 */
	public function setCompanyDetails(\App\Http\Vtiger_Request $request)
	{
		$details = [];
