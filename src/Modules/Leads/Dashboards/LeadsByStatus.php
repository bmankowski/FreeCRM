...
class LeadsByStatus  extends \App\Modules\Base\Views\Index
{
	...
	public function getLeadsByStatus($owner, $dateFilter)
	{
		...
		while ($row = $dataReader->read()) {
			$data[$i]['label'] = \App\Runtime\Vtiger_Language_Handler::translate($row['leadstatusvalue'], 'Leads');
			$ticks[$i][0] = $i;
			$ticks[$i][1] = \App\Runtime\Vtiger_Language_Handler::translate($row['leadstatusvalue'], 'Leads');
			$data[$i]['data'][0][0] = $i;
