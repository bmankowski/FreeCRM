...
class RelatedList extends \App\Modules\Base\Views\RelatedList
{
    ...
    public function process(\App\Http\Vtiger_Request $request)
    {
        ...
        //&[[["recruitment_status_rel","c","PPL_APPLIED##PPL_REJECTED_AFTER_CV"]]]
//		Tu jest dobry przykład jak to powinno wyglądać:
//		https://192.168.2.230/index.php?&page=1&cvId=relation&totalCount=0&entityState=Active&search_params=%5B%5B%5B%22recruitment_status_rel%22%2C%22c%22%2C%22PPL_APPLIED%23%23PPL_REJECTED_AFTER_CV%22%5D%5D%5D&view=Detail&module=ProjektyRekrutacyjne&record=1376448&relatedModule=Kandydaci&relatedView=ListPreview&mode=showRelatedList&tab_label=Kandydaci

        //@todo BMN returns error for custom fields, so it's commented
//		$searchParams = App\Condition::validSearchParams($relationListView->getQueryGenerator()->getModule(), $request->getArray('search_params'));
//		$searchParams=[[["recruitment_status_rel","c","PPL_CANDIDATE_PASSED_SCREENING"]]];

//   Filtering by custom field doesn't reload the page, but changing the relation_status does
...
//		$relationListView->set("parentRecord", $parentId);
        $models = $relationListView->getEntries($pagingModel);
        $header = $relationListView->getHeaders();
        $relationModel = $relationListView->getRelationModel();
        $relationModel->set('parentRecordId', $parentId);
