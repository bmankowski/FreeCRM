...
class DashBoard extends \App\Runtime\BaseModel
{
	...
	public function getDashboards($action = 1)
	{
		...
		while ($row = $dataReader->read()) {
			...
			if ($row['linklabel'] == 'Mini List') {
				...
				if (!$row['isdefault'])
					$row['deleteFromList'] = true;
				$charFilterWidget = \App\Modules\Base\Models\Widget::getInstanceFromValues($row);
				...
				$widgets[] = \App\Modules\Base\Models\Widget::getInstanceFromValues($row);
		}
	 ...
	 * @return \App\Modules\Base\Models\Widget[]
	 */
	 ...
	 * @return \App\Modules\Base\Models\DashBoard
	 */
