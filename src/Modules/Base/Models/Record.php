...
class Record extends \App\Runtime\BaseModel
{
	...
	public function isWatchingRecord()
	{
		if (!isset($this->isWatchingRecord)) {
			$watchdog = \App\Modules\Base\Models\Watchdog::getInstanceById($this->getId(), $this->getModuleName());
	...
	{
		return isset($this->rawData) ? $this->rawData : [];
	}
	...
	{
		$moduleName = $this->getModuleName();
		$path = "src/Modules/$moduleName/summary_blocks";
		if (!is_dir($path)) {
			return [];
		}
		$summaryBlocks = [];
	...
	public function setRecordFieldValues($parentRecordModel)
	{
		...
		if ($mfInstance) {
			...
			if ($parentRecordModel->getModule()->isInventory() && $this->getModule()->isInventory()) {
				$inventoryFieldModel = \App\Modules\Base\Models\InventoryField::getInstance($parentRecordModel->getModuleName());
