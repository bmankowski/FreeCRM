...
class DetailView extends \App\Runtime\BaseModel
{
	 ...
	 * @param \App\Modules\Base\Models\Module $moduleInstance - module model
	...
	public function getDetailViewLinks($linkParams)
	{
		...
		if ($recordModel->isEditable() && !empty($fieldToupdate)) {
			foreach ($fieldToupdate as $fieldLabel => $fieldName) {
				if (\App\Fields\Field::getFieldPermission($moduleName, $fieldName)) {
					...
					$linkModelList['DETAILVIEW'][] = \App\Modules\Base\Models\Link::getInstanceFromValues($editViewLinks);
				}
			}
		}
		...
		if ($moduleModel->isPermitted('ExportPdf')) {
			...
			if ($pdfModel->checkActiveTemplates($recordId, $moduleName, 'Detail')) {
				...
				$linkModelList['DETAILVIEW'][] = \App\Modules\Base\Models\Link::getInstanceFromValues($pdfLink);
			}
		}

		$relatedLinks = $this->getDetailViewRelatedLinks();
		foreach ($relatedLinks as &$relatedLinkEntry) {
			$relatedLink = \App\Modules\Base\Models\Link::getInstanceFromValues($relatedLinkEntry);
			$linkModelList[$relatedLink->getType()][] = $relatedLink;
		}

		$allLinks = \App\Modules\Base\Models\Link::getAllByType($moduleModel->getId(), ['DETAILVIEWBASIC', 'DETAILVIEW', 'DETAIL_VIEW_HEADER_WIDGET', 'DETAILVIEWTAB'], $linkParams);
		if (!empty($allLinks)) {
	...
	public function getSideBarLinks($linkParams, ?\App\Modules\Users\Models\Record $currentUser = null)
	{
		...
		$listLinks = \App\Modules\Base\Models\Link::getAllByType($this->getModule()->getId(), $listLinkTypes);
