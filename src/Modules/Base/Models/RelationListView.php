...
class RelationListView extends \App\Runtime\BaseModel
{
	 ...
	 * @return \App\Modules\Base\Models\Record[]
	 */
	public function getEntries(\App\Modules\Base\Models\Paging $pagingModel)
	{
		$relationModel = $this->getRelationModel();
	 ...
	 */
	public function getEntryExtend(\App\Modules\Base\Models\Record $recordModel)
	{
	...
	/**
	 * Get tree view model
	 * @return \App\Modules\Base\Models\TreeCategoryModal
	 */
	public function getTreeViewModel()
	{
		return \App\Modules\Base\Models\TreeCategoryModal::getInstance($this->getRelatedModuleModel());
	}
	...
	public function getTreeEntries()
	{
		...
		foreach ($rows as &$row) {
			...
			if ($showCreatorDetail) {
				...
				$tree['rel_created_time'] = \App\Modules\Base\UiTypes\Datetime::getDisplayDateTimeValue($row['rel_created_time']);
			}
			if ($showComment) {
	...
	public function getSelectRelationLinks()
	{
		...
		foreach ($selectLinkList as $selectLink) {
			$selectLinkModel[] = \App\Modules\Base\Models\Link::getInstanceFromValues($selectLink);
		}
		return $selectLinkModel;
	}
	...
	public function getAddRelationLinks()
	{
		...
		foreach ($addLinkList as &$addLink) {
			$addLinkModel[] = \App\Modules\Base\Models\Link::getInstanceFromValues($addLink);
		}
		return $addLinkModel;
	}
