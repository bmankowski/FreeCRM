...
class Inventory {
	 ...
	 * @return \App\Modules\Base\Models\Inventory instance
	 */
	...
	public function getAccountDiscount($relatedRecord)
	{
		...
		if (!empty($relatedRecord)) {
			$accountRecordModel = \App\Modules\Base\Models\Record::getInstanceById($relatedRecord);
	...
	public function getAccountTax($moduleName, $record)
	{
		...
		$accountTaxs = [];
		$name = '';
		$taxField = \App\Modules\Base\Models\InventoryField::getTaxField('Accounts');
		if ($accountField != '' && $taxField != false) {
			...
			if ($relationFieldValue != 0) {
				$accountRecordModel = \App\Modules\Base\Models\Record::getInstanceById($relationFieldValue, 'Accounts');
				$accountTaxs = \App\Modules\Base\UiTypes\Taxes::getValues($accountRecordModel->get($taxField));
				$name = $accountRecordModel->getName();
			}
		}
