...
class TransferOwnership extends \App\Runtime\BaseModel
{
	...
	public function getRelatedModuleRecordIds(\App\Http\Vtiger_Request $request, $recordIds = [], $relModData)
	{
		...
		$parentModuleModel = \App\Modules\Base\Models\Module::getInstance($basicModule);
		$relatedIds = [];
		...
		switch ($type) {
			...
			case 2:
				foreach ($recordIds as $recordId) {
					$recordModel = \App\Modules\Base\Models\Record::getInstanceById($recordId, $basicModule);
					$relationListView = \App\Modules\Base\Models\RelationListView::getInstance($recordModel, $relatedModule);
