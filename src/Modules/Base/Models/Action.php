...
class Action extends \App\Runtime\BaseModel
{
	...
	{
		$className = '\App\Modules\Base\Models\Action';
		$actionName = $row['actionname'];
		if (!in_array($actionName, self::$standardActions)) {
			$className = '\App\Modules\Base\Models\Utility';
		}
	...
	public static function getAllBasic($configurable = false)
	{
		...
		if ($configurable) {
			$sql .= ' AND actionname NOT IN (' . \App\Utils\Utils::generateQuestionMarks(self::$nonConfigurableActions) . ')';
