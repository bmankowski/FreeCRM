...
class Notebook extends \App\Runtime\BaseModel
{
	...
	public static function getUserInstance($widgetId)
	{
		...
		$result = $db->pquery('SELECT * FROM vtiger_module_dashboard_widgets 
			...
			WHERE linktype = ? AND vtiger_module_dashboard_widgets.id = ? AND vtiger_module_dashboard_widgets.userid = ?', ['DASHBOARDWIDGET', $widgetId, (int) (\App\User\CurrentUser::getId() ?? 0)]);
		$self = new self();
