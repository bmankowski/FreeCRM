...
class DataAccess_unique_value
{
	...
	public function process($moduleName, $ID, $record_form, $config)
	{
		...
		if ($ID != 0 && $ID != '' && !array_key_exists($config['what1'], $record_form)) {
			$Record_Model = \App\Modules\Base\Models\Record::getInstanceById($ID, $moduleName);
			...
			foreach ($wheres1 as $where) {
				...
				for ($i = 0; $i < $num; $i++) {
					...
					if ($metadata['setype'] == $DestModuleName) {
						...
						$fieldlabel .= '<li><a target="_blank" href="index.php?module=' . $DestModuleName . '&view=Detail&record=' . $id . '"><strong>' . \App\Record::getLabel($id) . '</strong></a> (' . \vtlib\Functions::getOwnerRecordLabel($metadata['smownerid']) . ')' . $deletedLabel . ',</li>';
					}
				}
			}
		}
