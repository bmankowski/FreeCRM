...
class Inventory extends \App\Runtime\BaseActionController
{
	...
	public function checkLimits(\App\Http\Vtiger_Request $request)
	{
		...
		$limitField = \App\Modules\Base\Models\Field::getInstance($limitFieldName, $moduleInstance);
		$balanceField = \App\Modules\Base\Models\Field::getInstance($balanceFieldName, $moduleInstance);
		if (!$limitField->isActiveField() || !$balanceField->isActiveField()) {
			$response->setResult(['status' => true]);
			$response->emit();
			return;
		}
		$recordModel = \App\Modules\Base\Models\Record::getInstanceById($record, 'Accounts');
		$limitID = $recordModel->get($limitFieldName);
		$balance = $recordModel->get($balanceFieldName);
		if (!empty($limitID)) {
			$limit = \App\Modules\Base\UiTypes\InventoryLimit::getValues($limitID)['value'];
		} else {
			$response->setResult(['status' => true]);
			$response->emit();
			return;
	...
	public function getUnitPrice(\App\Http\Vtiger_Request $request)
	{
		...
		if (in_array($recordModule, ['Products', 'Services'])) {
			$recordModel = \App\Modules\Base\Models\Record::getInstanceById($record, $recordModule);
	...
	public function getRecordDetail($recordId, $currencyId, $moduleName, $fieldName)
	{
		...
		if ($autoCompleteField) {
			foreach ($autoCompleteField as $field) {
				$moduleModel = \App\Modules\Base\Models\Module::getInstance($field['module']);
				$fieldModel = \App\Modules\Base\Models\Field::getInstance($field['field'], $moduleModel);
