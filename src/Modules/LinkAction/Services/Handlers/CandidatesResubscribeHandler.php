...
final class CandidatesResubscribeHandler implements HandlerInterface
{
	...
	public function handle(array $payload): void
	{
		...
		if ($recordId <= 0 || !LinkActionConfig::isEmailFieldAllowed($moduleName, $emailField)) {
			throw new \RuntimeException('Invalid Candidates resubscribe payload');
		}
		...
		$expiresAt = gmdate('Y-m-d', strtotime('+3 years'));
		PrivacyConsentWriter::grant($recordId, 'resubscribe_link', $expiresAt, null, $jti !== '' ? $jti : null);
	}
