		...
		if ($hierarchyAll && $vatId) {
			...
			while ($row = $db->getRow($result)) {
				if ($row['accountname'] == $accountName) {
					...
					$save = false;
					$fieldlabel .= '<li><a target="_blank" href="index.php?module=Accounts&view=Detail&record=' . $row['accountid'] . '"><strong>' . \App\Record::getLabel($row['accountid']) . '</strong></a> (' . \App\Fields\Owner::getLabel($metaData['smownerid']) . '),</li>';
				}
			}
		}
		if (!$hierarchyCheck) {
			...
			while ($id = $db->getSingleValue($result)) {
				...
				$fieldlabel .= '<li><a target="_blank" href="index.php?module=Accounts&view=Detail&record=' . $id . '"><strong>' . \App\Record::getLabel($id) . '</strong></a> (' . \App\Fields\Owner::getLabel($metaData['smownerid']) . ')' . $deletedLabel . ',</li>';
			}
		}
