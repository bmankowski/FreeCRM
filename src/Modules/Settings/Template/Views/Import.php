	...
	public function process(\App\Http\Vtiger_Request $request)
	{
		...
		if ($request->has('upload') && $request->get('upload') == 'true') {
			...
			if ($xmlError == UPLOAD_ERR_OK && $extension === 'xml') {
				...
				\App\Modules\Settings\Template\Models\Record::save($pdfModel, 'import');
				...
				if ($pdfModel->getId() && $imagePath != '' && $base64Image) {
					...
					\App\Modules\Settings\Template\Models\Record::save($pdfModel, 8);
		...
		$viewer->assign('PDF_DEFAULT_URL', \App\Modules\Settings\Template\Models\Module::getDefaultUrl());
