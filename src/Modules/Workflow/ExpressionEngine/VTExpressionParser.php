...
/**
 * Expression parser for workflow expressions
 */
class VTExpressionParser
{
	...
	public function consume(string $label, $value): void
	{
	...
	public function consumeSymbol(string $sym): void
	{
		$this->consume('SYMBOL', new VTExpressionSymbol($sym));
	}

	public function check($token, string $label, $value): bool
	{
		return $token->label == $label && $token->value == $value;
	}

	public function checkSymbol($token, string $sym): bool
	{
	...
	public function atom()
	{
		...
		switch ($token->label) {
			...
			case "OPEN_BRACKET":
				...
				return $val;
			default:
				throw new Exception("Unexpected token: " . $token->label);
		}
	}
	...
	private function binOpPrec(int $prec)
	{
		if ($prec >= 0) {
