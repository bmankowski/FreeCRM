...
class VTWorkflowManager {
	...
	{
		$query = (new \App\Db\Query())->from('com_vtiger_workflows')->where(['execution_condition' => \App\Modules\Workflow\VTWorkflowManager::$ON_SCHEDULE]);
		if ($referenceTime) {
	...
	public function getScheduledWorkflowsCount()
	{
		...
		$params = array(\App\Modules\Workflow\VTWorkflowManager::$ON_SCHEDULE);
