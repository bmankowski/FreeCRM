...
final class ActivityStateTask extends AbstractCronTask
{
	public function execute(): void
	{
		...
		$dataReader = (new \App\Db\Query())->select(['vtiger_activity.activityid', 'vtiger_activity.due_date', 'vtiger_activity.time_end',
			...
			->limit(\App\Core\AppConfig::module('Calendar', 'CRON_MAX_NUMBERS_ACTIVITY_STATE'))
			->createCommand()->query();
		while ($row = $dataReader->read()) {
			$state = \App\Modules\Calendar\Models\Module::getCalendarState($row);
			if ($state && $state != $row['activitystatus']) {
				$recordModel = \App\Modules\Base\Models\Record::getInstanceById($row['activityid']);
