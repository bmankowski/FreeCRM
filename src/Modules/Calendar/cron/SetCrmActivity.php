...
$dataReader = (new \App\Db\Query())->select(['vtiger_crmentity.crmid', 'vtiger_crmentity.setype'])
		...
		->limit(\FreeCRM\AppConfig::module('Calendar', 'CRON_MAX_NUMBERS_ACTIVITY_STATS'))
		->createCommand()->query();
while ($row = $dataReader->read()) {
	\FreeCRM\Modules\Calendar\Models\Record::setCrmActivity(array_flip([$row['crmid']]), $row['setype']);
}
