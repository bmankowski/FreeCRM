	...
	public static function importNewCandidates()
	{
		...
		foreach ($jsonFiles as $jsonFilePath) {
			try {
				...
				$application = self::getApplicationData($directory, $jsonFilePath, $candidateApplicationNumber);
		...
		foreach ($filesToProcess as $filePath) {
			try {
				...
				if (count($parts) >= 3) {
					...
					self::vecho("Nieprawidłowy format nazwy pliku: $fileName");
					continue;
				}
...
//				self::moveFilesToFailed($application);
	...
	private static function sendErrorMail($application, \Throwable $e): void
	{
	...
	static public function addCVToCandidate($candidate, $application)
	{
		...
		if (file_exists($originalFilename)) {
			try {
				...
				$fileContent = substr(\App\Utils\DocumentParser::parseFromFile($originalFilename), 0, 10000);
			} catch (\Exception $e) {
				self::vecho("Błąd podczas parsowania pliku" . $e->getMessage());
				$fileContent = "";
			}
