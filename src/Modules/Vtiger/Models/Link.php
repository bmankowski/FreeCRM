...
class Link extends \vtlib\Link
{
	 ...
	 * @return \App\Modules\Vtiger\Models\Link instance
	 */
	 ...
	 * @param \App\Modules\Vtiger\Models\Link $link - link model
	 * @result \App\Modules\Vtiger\Models\Link - current Instance;
	 */
	public function addChildLink(\App\Modules\Vtiger\Models\Link $link)
	{
	 ...
	 * @result <array> - list of \App\Modules\Vtiger\Models\Link instances
	 */
	...
	public function convertToNativeLink()
	{
		...
		if (!$this->isPageLoadLink()) {
			...
			$url = \App\Modules\Vtiger\helpers\Util::toSafeHTML($url);
			return $url;
		}
		$module = $parent = false;
		$sourceModule = false;
		...
		if (!empty($module) && !empty($sourceModule) && !empty($sourceRecord) && empty($parent)) {
			...
			$relatedModuleModel = \App\Modules\Vtiger\Models\Module::getInstance($module);
