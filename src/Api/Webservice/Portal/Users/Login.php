 ...
 */
class Login extends \App\Api\Webservice\Core\BaseAction
{
	...
	public function post()
	{
		...
		$row = (new \App\Db\Query())
				...
				->limit(1)->one($db);
		if (!$row) {
			throw new \App\Api\Webservice\Core\Exception('Invalid data access', 401);
		}
		if ($row['password_t'] !== $this->controller->request->get('password')) {
			throw new \App\Api\Webservice\Core\Exception('Invalid user password', 401);
		}
		$db->createCommand()
