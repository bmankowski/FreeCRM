...
class EmailTemplate
{
					...
					if (isset($values[$fieldName]) &&
						...
						if (strcasecmp($webserviceField->getFieldDataType(), 'reference') === 0) {
							...
							if (count($details) == 1) {
								...
								$type = \App\Records\Record::getType(
										$values[$fieldName]);
	...
	{
		if (\App\Utils\ModuleUtils::isModuleActive($module) && \App\Security\Privilege::isPermitted($module, 'EditView')) {
			return true;
		}
		return false;
	}
	...
	public function isActive($field, $mod)
	{
		...
		$tabid = \App\Utils\ModuleUtils::getModuleId($mod);
