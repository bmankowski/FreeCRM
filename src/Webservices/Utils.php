...
function vtws_transferOwnership($ownerId, $newOwnerId, $delete = true)
{
	...
	$newOwnerModel = \App\Modules\Users\Models\Record::getInstanceById($newOwnerId, 'Users');
...
function vtws_transferOwnershipForWorkflowTasks($ownerModel, $newOwnerModel)
{
	...
	while ($row = $dataReader->read()) {
		...
		if (array_key_exists('field_value_mapping', $unserializeTask)) {
			$fieldMapping = \App\Utils\Json::decode($unserializeTask->field_value_mapping);
			if (!empty($fieldMapping)) {
				...
				$updatedTask = \App\Utils\Json::encode($fieldMapping);
