...
function vtws_history($element, $user)
{
	...
	if (!CRMEntity::getInstance('ModTracker') || !\App\Utils\ModuleUtils::isModuleActive('ModTracker')) {
	...
	if (!empty($orderedIds)) {
		...
		$sql .= sprintf(' WHERE vtiger_modtracker_detail.id IN (%s)', \App\Utils\Utils::generateQuestionMarks($orderedIds));
...
function vtws_history_entityIdHelper($moduleName, $id)
{
	...
	if (!isset($wsEntityIdCache[$moduleName][$id])) {
		...
		if (empty($moduleName)) {
			$moduleName = \App\Records\Record::getType($id);
		}
