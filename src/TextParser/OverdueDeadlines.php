...
class OverdueDeadlines extends Base
{
	...
	$queryGenerator = new \App\QueryGenerator($moduleName, $adminUser);
		...
		foreach ($this->columnNames as $column) {
			...
			$html .= '<th><span>' . \App\Runtime\Vtiger_Language_Handler::translate($fieldModel->get('label'), $moduleName) . '</span>&nbsp;</th>';
		}
		$html .= '</tr></thead><tbody>';
			...
			foreach ($this->columnNames as $column) {
				...
				if ($column == 'link') {
					...
					if (!empty($linkId) && \App\Utils\Utils::isRecordExists($linkId)) {
						$processRecordModel = \App\Modules\Base\Models\Record::getInstanceById($linkId);
