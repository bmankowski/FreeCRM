...
class ProductsTableRelatedModule extends Base
{
	...
	public function process()
	{
		...
		$inventoryField = \App\Modules\Vtiger\Models\InventoryField::getInstance($relatedModuleName);
		...
		if (count($fields[1]) != 0) {
			...
			foreach ($fields[1] as $field) {
				if ($field->isVisible($inventoryRows)) {
					$html .= '<th style="width:' . $field->get('colspan') . '%;" class="textAlignCenter tBorder tHeader">' . \App\Runtime\Vtiger_Language_Handler::translate($field->get('label'), $this->textParser->moduleName) . '</th>';
				}
			}
			$html .= '</tr></thead><tbody>';
