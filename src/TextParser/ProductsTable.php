		...
		if (count($fields[0]) != 0) {
			...
			foreach ($fields[0] as $field) {
				$html .= '<th style="' . $field->get('colspan') . '%;" class="tBorder noBottomBorder tHeader">
								<span>' . \App\Runtime\Vtiger_Language_Handler::translate($field->get('label'), $this->textParser->moduleName) . ':</span>&nbsp;';
				switch ($field->getTemplateName('DetailView', $this->textParser->moduleName)) {
					...
					case 'DetailViewDiscountMode.tpl':
						$html .= \App\Runtime\Vtiger_Language_Handler::translate($field->getDisplayValue($inventoryRows[0][$field->get('columnname')]), $this->textParser->moduleName);
						break;
				}
				$html .= '</th>';
			}
			...
			foreach ($fields[1] as $field) {
				if ($field->isVisible($inventoryRows)) {
					$html .= '<th style="' . $field->get('colspan') . '%;" class="textAlignCenter tBorder tHeader">' . \App\Runtime\Vtiger_Language_Handler::translate($field->get('label'), $this->textParser->moduleName) . '</th>';
				}
			}
			$html .= '</tr>
				</thead>
							...
							<thead>
								<tr>
									<th class="tBorder noBottomBorder tHeader">
										<strong>' . \App\Runtime\Vtiger_Language_Handler::translate('LBL_DISCOUNTS_SUMMARY', $this->textParser->moduleName) . '</strong>
									</th>
								</tr>
							</thead>
							<tbody>
			...
			if (in_array('tax', $columns) && in_array('taxmode', $columns)) {
							...
							<thead>
								<tr>
									<th colspan="2" class="tBorder noBottomBorder tHeader">
										<strong>' . \App\Runtime\Vtiger_Language_Handler::translate('LBL_TAX_SUMMARY', $this->textParser->moduleName) . '</strong>
									</th>
								</tr>
							</thead>
							<tbody>';
				...
				$html .= '<tr>
									<td class="textAlignRight tBorder" width="70px">' . \App\Runtime\Vtiger_Language_Handler::translate('LBL_AMOUNT', $this->textParser->moduleName) . '</td>
				...
				if (in_array('currency', $columns) && $baseCurrency['id'] != $currency) {
					...
					$html .= '<br /><table class="pTable colapseBorder">
								<thead>
									<tr>
										<th colspan="2" class="tBorder noBottomBorder tHeader">
											<strong>' . \App\Runtime\Vtiger_Language_Handler::translate('LBL_CURRENCIES_SUMMARY', $this->textParser->moduleName) . '</strong>
										</th>
									</tr>
								</thead>
								<tbody>';
