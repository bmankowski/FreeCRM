 ...
 * The Original Code is:  FreeCRM (based on Vtiger CRM Open Source)
 * All Rights Reserved.
 * Author: bmankowski@gmail.com
 * 
 * Home Index view JavaScript controller - extends Vtiger_DashBoard_Js functionality
 ************************************************************************************/

jQuery.Class("Home_Index_Js", {
	gridster: false,
	...
	addWidget: function (element, url) {
		...
		Home_Index_Js.gridster.add_widget(widgetContainer, width, height);
		Home_Index_Js.currentInstance.loadWidget(widgetContainer);
	},
	...
	instancesCache: {},
	init: function () {
		Home_Index_Js.currentInstance = this;
	},
	...
	registerGridster: function () {
		...
		Home_Index_Js.gridster = this.getContainer().gridster({
			widget_margins: [7, 7],
	...
	gridsterStop: function () {
		var gridster = Home_Index_Js.gridster;
								...
								function (response) {
									if (response.success) {
										...
										if (jQuery.inArray(widgetName, nonReversableWidgets) == -1) {
											Home_Index_Js.gridster.remove_widget(element.closest('li'));
											jQuery('.widgetsList').prev('button').css('visibility', 'visible');
											var data = '<li><a onclick="Home_Index_Js.addWidget(this, \'' + response.result.url + '\')" href="javascript:void(0);"';
		...
		thisInstance.saveWidget(paramsForm, 'save', sourceModule).then(
				function (data) {
					...
					if (data['success']) {
						...
						Home_Index_Js.addWidget(linkElement, 'index.php?module=Home&view=ShowWidget&name=ChartFilter&linkid=' + element.data('linkid') + '&widgetid=' + result['wid'] + '&active=0');
		...
		thisInstance.saveWidget(paramsForm, 'save', sourceModule).then(
				function (data) {
					...
					if (data['success']) {
						...
						linkElement.data('id', result['wid']);
						Home_Index_Js.addWidget(linkElement, 'index.php?module=Home&view=ShowWidget&name=MiniList&linkid=' + element.data('linkid') + '&widgetid=' + result['wid'] + '&active=0');
						Vtiger_Helper_Js.showMessage(params);
					} else {
						var message = data['error']['message'];
						if (data['error']['code'] != 513) {
