...
Settings_Template_Edit_Js("Settings_Template_Edit6_Js", {}, {
		...
		AppConnector.request(saveData).then(
				function (data) {
					...
					if (data.success == true) {
						...
						setTimeout(function () {
							window.location.href = "index.php?module=Template&parent=Settings&page=1&view=ListView";
	...
	registerUploadButton: function (form) {
		form.find('#uploadWM').on('click', function (e) {
			...
			// Open the connection.
			xhr.open('POST', 'index.php?module=Template&parent=Settings&action=Watermark&mode=Upload', true);
