...
Settings_Vtiger_ListView_Js("Settings_Template_ListView_Js", {
	deletePdfTemplate: function (templateId, event) {
		...
		Settings_Template_ListView_Js.getInstance().deleteTemplate(templateId);
		return false;
	}
}, {
	...
	registerRowClickEvent: function () {
		...
		listViewContentDiv.off('click.pdfRow').on('click.pdfRow', '.listViewEntries', function (e) {
			...
			if (!editUrl) {
				...
				if (recordId) {
					editUrl = 'index.php?module=' + app.getModuleName() + '&parent=' + app.getParentModuleName() + '&view=Edit&record=' + recordId;
				}
			}
			if (editUrl) {
...
Settings_Template_ListView_Js.deletePdfTemplate = function (templateId, event) {
	if (event) {
		if (typeof event.stopPropagation === 'function') {
			event.stopPropagation();
		} else {
