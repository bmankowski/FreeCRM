...
Vtiger_Edit_Js("DocumentTemplates_Edit_Js", {
	instance: {},
	prepareSaveRequestData: function (form, step) {
		var data = form.serializeFormData();
		data.module = app.getModuleName() || 'DocumentTemplates';
		data.action = 'Save';
		data.step = step;
		data.async = false;
		delete data.view;
	...
	parseSaveResponse: function (data) {
		if (typeof data === 'string') {
			data = JSON.parse(data);
		}
		return data;
	},
	hideProgress: function (progressIndicatorElement) {
		if (progressIndicatorElement) {
			progressIndicatorElement.progressIndicator({ mode: 'hide' });
		}
	},
	submitSaveAndLoadNext: function (form, step, progressIndicatorElement, aDeferred) {
		var saveData = DocumentTemplates_Edit_Js.prepareSaveRequestData(form, step);
		AppConnector.request(saveData).then(
			function (data) {
				data = DocumentTemplates_Edit_Js.parseSaveResponse(data);
				if (data.success == true) {
					...
					var loadData = {
						module: app.getModuleName() || 'DocumentTemplates',
						view: 'Edit',
						mode: 'Step' + (parseInt(step, 10) + 1),
						record: data.result.id
					};
					AppConnector.request(loadData).then(
						function (html) {
							form.hide();
	...
	getInstance: function (step) {
		step = String(step).replace(/^step/i, '');
		if (step in DocumentTemplates_Edit_Js.instance) {
			return DocumentTemplates_Edit_Js.instance[step];
		}
		var stepClassMap = {
			'3TemplateGeneratorFooter': 'DocumentTemplates_Edit3_Js',
			'3TemplateTwoSnippets': 'DocumentTemplates_Edit3_Js'
		};
		var moduleClassName = stepClassMap[step] || ('DocumentTemplates_Edit' + step + '_Js');
		if (typeof window[moduleClassName] !== 'function') {
			app.errorLog('PDF wizard step class not found: ' + moduleClassName);
			return null;
		}
		DocumentTemplates_Edit_Js.instance[step] = new window[moduleClassName]();
		return DocumentTemplates_Edit_Js.instance[step];
	},
	/*
	...
	initiate: function (container) {
		...
		this.currentInstance.initialize(this.getStepContainer(stepVal));
		this.registerEvents();
...
jQuery(function () {
	var contents = jQuery('.pdfTemplateContents');
	if (!contents.length || typeof DocumentTemplates_Edit_Js === 'undefined') {
		return;
	}
	if (contents.data('pdfWizardInitialized')) {
		return;
	}
	contents.data('pdfWizardInitialized', true);
	var editInstance = new DocumentTemplates_Edit_Js();
	editInstance.initiate(contents.first());
});
