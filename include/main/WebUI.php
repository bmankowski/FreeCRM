...
class WebUI extends Vtiger_EntryPoint
{
	 ...
	 */
	protected function triggerCheckPermission($handler, Vtiger_Request $request)
	{
		...
		// Call checkPermission if available
		if (method_exists($handler, 'checkPermission')) {
	...
	{
		$handlerClass = Vtiger_Loader::getComponentClassName(
			$componentType,
			$componentName,
			$qualifiedModuleName
		);
