	...
	<div class="modal-body row">
		<div class="col-md-12">
			<form class="form-horizontal fieldDetailsForm sendByAjax validateForm" method="POST">
				...
				<strong>{'LBL_LABEL_NAME'|t:$QUALIFIED_MODULE}:&nbsp;</strong>{$FIELD_MODEL->getFieldLabel()|t:$SELECTED_MODULE_NAME}<br>
				<strong>{'LBL_FIELD_NAME'|t:$QUALIFIED_MODULE}:&nbsp;</strong>{$FIELD_MODEL->getFieldName()}
				...
				<div class="checkbox">
					...
					<label>
						...
						{'LBL_MANDATORY_FIELD'|t:$QUALIFIED_MODULE}
					</label>
				</div>
				<div class="checkbox">
				...
				<div class="checkbox">
					...
					<label>
						...
						{'LBL_QUICK_CREATE'|t:$QUALIFIED_MODULE}
					</label>
				</div>
				<div class="checkbox">
					...
					<label>
						...
						{'LBL_SUMMARY_FIELD'|t:$QUALIFIED_MODULE}
					</label>
				</div>
				<div class="checkbox">
					...
					<label>
						...
						{'LBL_HEADER_FIELD'|t:$QUALIFIED_MODULE}
					</label>
				</div>
				<div class="checkbox">
					...
					<label>
						...
						{'LBL_MASS_EDIT'|t:$QUALIFIED_MODULE}
					</label>
				</div>
				...
				<div class="checkbox">
					...
					<label>
						...
						{'LBL_DEFAULT_VALUE'|t:$QUALIFIED_MODULE}
					</label>
							...
							{if $FIELD_MODEL->getFieldDataType() eq "picklist"}
								...
								<select class="col-md-2 select2" name="fieldDefaultValue" {if !$FIELD_MODEL->hasDefaultValue()} disabled="" {/if} data-validation-engine="validate[required,funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" data-fieldinfo='{Vtiger_Util_Helper::toSafeHTML(\App\Json::encode($FIELD_INFO))}'>
									{foreach item=PICKLIST_VALUE key=PICKLIST_NAME from=$PICKLIST_VALUES}
										<option value="{Vtiger_Util_Helper::toSafeHTML($PICKLIST_NAME)}" {if decode_html($FIELD_MODEL->get('defaultvalue')) eq $PICKLIST_NAME} selected {/if}>{|$PICKLIST_VALUE|t:$SELECTED_MODULE_NAME}</option>
									{/foreach}
								</select>
							{elseif $FIELD_MODEL->getFieldDataType() eq "multipicklist"}
								...
								<select multiple class="col-md-2 select2" data-validation-engine="validate[required,funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" {if !$FIELD_MODEL->hasDefaultValue()} disabled="" {/if}  name="fieldDefaultValue" data-fieldinfo='{Vtiger_Util_Helper::toSafeHTML(\App\Json::encode($FIELD_INFO))}'>
									{foreach item=PICKLIST_VALUE from=$PICKLIST_VALUES}
										<option value="{Vtiger_Util_Helper::toSafeHTML($PICKLIST_VALUE)}" {if in_array(Vtiger_Util_Helper::toSafeHTML($PICKLIST_VALUE), $FIELD_VALUE_LIST)} selected {/if}>{|$PICKLIST_VALUE|t:$SELECTED_MODULE_NAME}</option>
									{/foreach}
								</select>
				...
				{if in_array($FIELD_MODEL->getFieldDataType(),['string','phone','currency','url','integer','double'])}
					<div>
						<strong>{'LBL_FIELD_MASK'|t:$QUALIFIED_MODULE}</strong>&nbsp;
						<div class="marginLeft20 input-group marginBottom10px">
							...
							<span class="input-group-addon"><span class="glyphicon glyphicon-info-sign popoverTooltip" data-placement="top" data-content="{'LBL_FIELD_MASK_INFO'|t:$QUALIFIED_MODULE}"></span></span>
						</div>
					</div>
				{/if}
				<div>
					<strong>{'LBL_MAX_LENGTH_TEXT'|t:$QUALIFIED_MODULE}</strong>
					<div class="marginLeft20">
						<input type="text" class="form-control" name="maxlengthtext" value="{$FIELD_MODEL->get('maxlengthtext')}" />&nbsp;
					</div>
				</div>
				<div>
					<strong>{'LBL_MAX_WIDTH_COLUMN'|t:$QUALIFIED_MODULE}</strong>
					<div class="marginLeft20">
				...
				{if AppConfig::developer('CHANGE_GENERATEDTYPE')}
					<div class="checkbox">
						<label>
							...
							{'LBL_GENERATED_TYPE'|t:$QUALIFIED_MODULE}
						</label>
					</div>
				{/if}
