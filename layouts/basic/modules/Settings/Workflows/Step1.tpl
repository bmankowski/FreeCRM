...
*************************************************************************************}
{strip}
<!-- layouts/basic/modules/Settings/Workflows/Step1.tpl -->
    <div class="workFlowContents">
        <form name="EditWorkflow" action="index.php" method="post" id="workflow_step1" class="form-horizontal">
            ...
            <div class="padding1per" style="border:1px solid #ccc;">
                <label>
                    <strong>{"LBL_STEP_1"|t:$QUALIFIED_MODULE}: {"LBL_ENTER_BASIC_DETAILS_OF_THE_WORKFLOW"|t:$QUALIFIED_MODULE}</strong>
                </label>
                <br>
                <div class="form-group">
                    ...
                    <div class="col-sm-6 controls">
                        {if $MODE eq 'edit'}
                            <input type='text' disabled='disabled' class="form-control" value="{$MODULE_MODEL->getName()|t:$MODULE_MODEL->getName()}" >
                ...
                <div class="form-group">
                    <label class="col-sm-3 control-label">
                        {"LBL_DESCRIPTION"|t:$QUALIFIED_MODULE}<span class="redColor">*</span>
                    </label>
                    <div class="col-sm-6 controls">
                ...
                <div class="form-group">
                    <label class="col-sm-3 control-label">
                        {"LBL_SPECIFY_WHEN_TO_EXECUTE"|t:$QUALIFIED_MODULE}
                    </label>
                    <div class="col-sm-6 controls">
                        ...
                        {if $SCHEDULED_WORKFLOW_COUNT <= $MAX_ALLOWED_SCHEDULED_WORKFLOWS}
                            <div id="scheduleBox" class='well contentsBackground {if $WORKFLOW_MODEL_OBJ->executionCondition neq 6} hide {/if}'>
                                <div class='row'>
                                    <div class="col-md-2" style='position:relative;top:5px;'>{"LBL_RUN_WORKFLOW"|t:$QUALIFIED_MODULE}</div>
                                    <div class="col-md-6">
										<select class='chzn-select' id='schtypeid' name='schtypeid'>
                                            <option value="1" {if $WORKFLOW_MODEL_OBJ->schtypeid eq 1}selected{/if}>{"LBL_HOURLY"|t:$QUALIFIED_MODULE}</option>
                                            <option value="2" {if $WORKFLOW_MODEL_OBJ->schtypeid eq 2}selected{/if}>{"LBL_DAILY"|t:$QUALIFIED_MODULE}</option>
                                            <option value="3" {if $WORKFLOW_MODEL_OBJ->schtypeid eq 3}selected{/if}>{"LBL_WEEKLY"|t:$QUALIFIED_MODULE}</option>
                                            <option value="4" {if $WORKFLOW_MODEL_OBJ->schtypeid eq 4}selected{/if}>{"LBL_SPECIFIC_DATE"|t:$QUALIFIED_MODULE}</option>
                                            <option value="5" {if $WORKFLOW_MODEL_OBJ->schtypeid eq 5}selected{/if}>{"LBL_MONTHLY_BY_DATE"|t:$QUALIFIED_MODULE}</option>
                                            <!--option value="6" {if $WORKFLOW_MODEL_OBJ->schtypeid eq 6}selected{/if}>{"LBL_MONTHLY_BY_WEEKDAY"|t:$QUALIFIED_MODULE}</option-->
                                            <option value="7" {if $WORKFLOW_MODEL_OBJ->schtypeid eq 7}selected{/if}>{"LBL_YEARLY"|t:$QUALIFIED_MODULE}</option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class='row {if $WORKFLOW_MODEL_OBJ->schtypeid neq 4} hide {/if}' id='scheduleByDate' style="padding:5px 0px;">
                                    <div class="col-md-2" style='position:relative;top:5px;'>{"LBL_CHOOSE_DATE"|t:$QUALIFIED_MODULE}</div>
                                    <div class="col-md-6">
                                        <div class="date">
								...
								<div class='row {if $WORKFLOW_MODEL_OBJ->schtypeid neq 7} hide {/if}' id='scheduleAnually' style='padding:5px 0px;'>
									<div class="col-md-2" style='position:relative;top:5px;'>
										{"LBL_SELECT_MONTH_AND_DAY"|t:$QUALIFIED_MODULE}
									</div>
									<div class="col-md-10">
									...
									<div class="col-md-10">
										<div style='padding-bottom:5px;'>{"LBL_SELECTED_DATES"|t:$QUALIFIED_MODULE}</div>
										<div>
								...
								<div class='row {if $WORKFLOW_MODEL_OBJ->schtypeid < 2} hide {/if}' id='scheduledTime' style='padding:5px 0px 10px 0px;'>
									<div class="col-md-2" style='position:relative;top:5px;'>
										{"LBL_AT_TIME"|t:$QUALIFIED_MODULE}
									</div>
