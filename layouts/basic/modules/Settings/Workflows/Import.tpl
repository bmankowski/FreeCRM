		...
		{if isset($UPLOAD)}
			...
			{elseif $UPLOAD eq false}
				<p class="bgMessage bgERROR">
					{"LBL_UPLOAD_ERROR"|t:$QUALIFIED_MODULE} <a href="{$DEFAULT_URL}">{"LBL_RETURN"|t:$QUALIFIED_MODULE}</a>
				</p>
			{/if}
		{else}
			<form name="ImportWorkflowTemplate" action="index.php" method="post" class="form-horizontal" enctype="multipart/form-data">
				...
				<div class="pull-right">
					...
					<a href="{$DEFAULT_URL}" class="btn btn-warning" type="button">{"LBL_CANCEL"|t:$QUALIFIED_MODULE}</a>
				</div>
			</form>
		{/if}
	</div>
