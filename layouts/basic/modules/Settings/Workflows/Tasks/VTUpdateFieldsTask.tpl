				...
				<span class="col-md-4">
					<select name="fieldname" class="chzn-select" style="min-width: 250px" data-placeholder="{"LBL_SELECT_FIELD"|t:$QUALIFIED_MODULE}">
						...
						{foreach from=$MODULE_MODEL->getFields() item=FIELD_MODEL}
							...
							<option value="{$FIELD_MODEL->get('name')}" {if $FIELD_MAP['fieldname'] eq $FIELD_MODEL->get('name')}selected=""{/if}data-fieldtype="{$FIELD_MODEL->getFieldType()}" data-field-name="{$FIELD_MODEL->get('name')}" data-fieldinfo="{\App\Modules\Vtiger\helpers\Util::toSafeHTML(\App\Json::encode($FIELD_INFO))}" >
		...
		<span class="col-md-4">
			<select name="fieldname" data-placeholder="{"LBL_SELECT_FIELD"|t:$QUALIFIED_MODULE}" class="form-control">
				...
				{foreach from=$MODULE_MODEL->getFields() item=FIELD_MODEL}
					...
					{assign var=FIELD_INFO value=$FIELD_MODEL->getFieldInfo()}
					{assign var=MODULE_MODEL value=$FIELD_MODEL->getModule()}
					<option value="{$FIELD_MODEL->get('name')}" data-fieldtype="{$FIELD_MODEL->getFieldType()}" data-field-name="{$FIELD_MODEL->get('name')}" data-fieldinfo="{\App\Modules\Vtiger\helpers\Util::toSafeHTML(\App\Json::encode($FIELD_INFO))}" >
						{if $SOURCE_MODULE neq $MODULE_MODEL->get('name')}
							({$MODULE_MODEL->get('name')|t:$MODULE_MODEL->get('name')})  {$FIELD_MODEL->get('label')|t:$MODULE_MODEL->get('name')}
						{else}
							{$FIELD_MODEL->get('label')|t:$SOURCE_MODULE}
