...
<!-- layouts/basic/modules/Settings/Workflows/Tasks/VTUpdateFieldsTask.tpl -->
	...
	<div class="row conditionsContainer" id="save_fieldvaluemapping">
		...
		<input type="hidden" id="fieldValueMapping" name="field_value_mapping" value='{\App\Modules\Base\Helpers\Util::toSafeHTML($TASK_OBJECT->field_value_mapping)}' />
				...
				<span class="col-md-4">
					<select name="fieldname" class="chzn-select" style="min-width: 250px" data-placeholder="{"LBL_SELECT_FIELD"|t:$QUALIFIED_MODULE}">
						...
						{foreach from=$MODULE_MODEL->getFields() item=FIELD_MODEL}
							...
							<option value="{$FIELD_MODEL->get('name')}" {if $FIELD_MAP['fieldname'] eq $FIELD_MODEL->get('name')}selected=""{/if}data-fieldtype="{$FIELD_MODEL->getFieldType()}" data-field-name="{$FIELD_MODEL->get('name')}" data-fieldinfo="{\App\Modules\Base\Helpers\Util::toSafeHTML(\App\Json::encode($FIELD_INFO))}" >
