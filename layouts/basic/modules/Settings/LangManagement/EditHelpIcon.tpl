...
<div class="">
	<div class="form-horizontal">
		<div class="form-group row">
			<label class="control-label col-md-1" >{"Language"|t:$QUALIFIED_MODULE}:</label>
			<div class="col-md-3">
			...
			<div class="checkbox col-md-2">
				<label class=""><input type="checkbox" class="show_differences" name="show_differences" {if $SD eq 1}checked{/if} value="1">{"LBL_SHOW_EMPTY_VALUES"|t:$QUALIFIED_MODULE}</label>
			</div>
		</div>
	</div>
</div>
	...
	<table class="table table-bordered table-condensed listViewEntriesTable" >
		<thead>
			<tr class="blockHeader">
				<th class="col-md-1"><strong>{"LBL_FIELD_NAME"|t:$QUALIFIED_MODULE}</strong></th>
				<th class="col-md-2"><strong>{"LBL_SHOW_IN"|t:$QUALIFIED_MODULE}</strong></th>
				...
				{if $TEMPDATA == 1}
					<tr data-langkey="{$lang_key}">
						{foreach from=$langs item=item key=key}
							...
							{elseif $key eq 'info'}
								<td  class="">
									<select class="helpInfoView form-control" id="helpInfoView" name="helpInfoView" multiple data-fieldid="{$item.fieldid}" placeholder="{"LBL_SELECT_OPTION"|t:$QUALIFIED_MODULE}">
											<option value="Edit" {if in_array('Edit',$item.view)}selected{/if}>{"LBL_EDIT"|t:$QUALIFIED_MODULE}</option>
											<option value="Detail" {if in_array('Detail',$item.view)}selected{/if}>{"LBL_DETAIL"|t:$QUALIFIED_MODULE}</option>
											<option value="QuickCreateAjax" {if in_array('QuickCreateAjax',$item.view)}selected{/if}>{"LBL_QUICKCREATE"|t:$QUALIFIED_MODULE}</option>
									</select>
								</td>
							{else}
								<td class="showText">
								<div>
								<button class="btn btn-default editButton">{"LBL_EDIT_RECORD"|t:$QUALIFIED_MODULE}</button>
