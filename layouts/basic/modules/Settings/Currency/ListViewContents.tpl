...
-->*}
{strip}
<!-- layouts/basic/modules/Settings/Currency/ListViewContents.tpl -->
										...
										<span class="actionImages">
											{foreach item=RECORD_LINK from=$LISTVIEW_ENTRY->getRecordLinks()}
												...
												<a {if stripos($RECORD_LINK_URL, 'javascript:')===0} onclick="{$RECORD_LINK_URL|substr:strlen("javascript:")};
													...
													<i class="{$RECORD_LINK->getIcon()} alignMiddle" title="{$RECORD_LINK->getLabel()|t:$QUALIFIED_MODULE}"></i>
												</a>
												{if !$RECORD_LINK@last}
													&nbsp;&nbsp;
												{/if}
		...
		{if $LISTVIEW_ENTRIES_COUNT eq '0'}
			<table class="emptyRecordsDiv">
				<tbody>
					<tr>
						<td>
							{"LBL_EQ_ZERO"|t} {$MODULE|t:$QUALIFIED_MODULE} {"LBL_FOUND"|t}
						</td>
					</tr>
				</tbody>
			</table>
