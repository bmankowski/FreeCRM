{strip}
<!-- layouts/basic/modules/Settings/Base/SystemWarningsList.tpl -->
	{*<!-- {[The file is published on the basis of YetiForce Public License that can be found in the following directory: licenses/License.html]} --!>*}
	<table class="table table-bordered table-condensed">
		<thead>
			<tr>
				<th>{'LBL_WARNINGS_TITLE'|t:$MODULE}</th>
				<th>{'LBL_WARNINGS_STATUS'|t:$MODULE}</th>
				<th>{'LBL_WARNINGS_PRIORITY'|t:$MODULE}</th>
				<th>{'LBL_WARNINGS_FOLDER'|t:$MODULE}</th>
				<th></th>
			</tr>
		</thead>
		<tbody class="notificationEntries">
			{foreach from=$WARNINGS_LIST item=ITEM}
				<tr data-id="{get_class($ITEM)}" data-status="{$ITEM->getStatus()}">
					<td>{$ITEM->getTitle()|t:'Settings:SystemWarnings'}</td>
					...
					<td class="text-center">
						{foreach from=$ITEM->getFolder() item=FOLDER name=FOLDERS}
							{$FOLDER|t:'Settings:SystemWarnings'}{if not $smarty.foreach.FOLDERS.last}/{/if} 
						{/foreach}
					</td>
					<td class="text-center">
						{if $ITEM->getStatus() != 1 && $ITEM->getPriority() < 8}
							<button class="btn btn-warning btn-xs setIgnore popoverTooltip" data-placement="top" data-content="
									{if $ITEM->getStatus() == 2}
										{'BTN_REMOVE_IGNORE'|t:'Settings:SystemWarnings'}
									{else}
									...
									<div class="modal-dialog">
										<div class="modal-content">
											<div class="modal-header">
												...
												<h4 class="modal-title" id="myModalLabel">{$ITEM->getTitle()|t:'Settings:SystemWarnings'}</h4>
											</div>
