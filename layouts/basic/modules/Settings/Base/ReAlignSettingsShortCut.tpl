...
-->*}
{strip}
<!-- layouts/basic/modules/Settings/Base/ReAlignSettingsShortCut.tpl --> {assign var=SPAN_COUNT value=1}
	<div  class="row">
		{foreach item=SETTING_SHORTCUT from=$SETTINGS_SHORTCUT name=shortcuts}
			<div id="shortcut_{$SETTING_SHORTCUT->getId()}" style="margin-left: 20px;" data-actionurl="{$SETTING_SHORTCUT->getPinUnpinActionUrl()}" class="col-md-3 contentsBackground well cursorPointer moduleBlock" data-url="{$SETTING_SHORTCUT->getUrl()}">
				...
				<h5 class="themeTextColor">{$SETTING_SHORTCUT->get('name')|t:\App\Modules\Base\Models\Menu::getModuleNameFromUrl($SETTING_SHORTCUT->getUrl())}</h5>
				<div>{$SETTING_SHORTCUT->get('description')|t:\App\Modules\Base\Models\Menu::getModuleNameFromUrl($SETTING_SHORTCUT->getUrl())}</div>
			</div>
			 {if $SPAN_COUNT==3}</div>{$SPAN_COUNT=1}{if not $smarty.foreach.shortcuts.last}<div class="row">{/if}{continue}{/if}
							{$SPAN_COUNT=$SPAN_COUNT+1}
        {/foreach}
	</div>
<!--/layouts/basic/modules/Settings/Base/ReAlignSettingsShortCut.tpl -->
{/strip}
