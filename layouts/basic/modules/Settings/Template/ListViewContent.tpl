											...
											{foreach from=$LINKS item=LINK}
												{if $LINK->getLabel() eq 'LBL_DELETE_RECORD'}
													<div class="btn-group {if $USER_MODEL->get('rowheight') eq 'narrow'}btn-group-sm{/if}">
														<button type="button" class="btn btn-default templateDelete popoverTooltip" data-id="{$LISTVIEW_ENTRY->getId()}" aria-label="{$LINK->getLabel()|t:$QUALIFIED_MODULE}" data-placement="bottom" data-content="{$LINK->getLabel()|t:$QUALIFIED_MODULE}" onclick="if (typeof event.stopPropagation === 'function') {ldelim}event.stopPropagation();{rdelim} else {ldelim}event.cancelBubble = true;{rdelim} if (typeof event.preventDefault === 'function') {ldelim}event.preventDefault();{rdelim} var recordId = {$LISTVIEW_ENTRY->getId()}; if (typeof Settings_Template_ListView_Js !== 'undefined' && Settings_Template_ListView_Js.deletePdfTemplate) {ldelim}return Settings_Template_ListView_Js.deletePdfTemplate(recordId, event);{rdelim} Vtiger_Helper_Js.showConfirmationBox({ldelim}message: app.vtranslate('LBL_DELETE_CONFIRMATION'){rdelim}).then(function () {ldelim}AppConnector.request({ldelim}module: app.getModuleName(), parent: app.getParentModuleName(), action: 'DeleteAjax', record: recordId{rdelim}).then(function () {ldelim}window.location.reload();{rdelim});{rdelim}); return false;">
