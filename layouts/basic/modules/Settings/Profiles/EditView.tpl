						...
						<div class="widget_header row">
							...
							<div class="col-md-4 btn-toolbar">
								<div class="pull-right">
									...
									<a class="cancelLink btn btn-warning" onclick="javascript:window.history.back();"
										type="reset"
										title="{"LBL_CANCEL"|t:$QUALIFIED_MODULE}">{"LBL_CANCEL"|t:$QUALIFIED_MODULE}</a>
								</div>
							</div>
						</div>
						...
						<div class="">
							<div class="row">
								<label class="col-md-2"><span
										class="redColor">*</span><strong>{"LBL_PROFILE_NAME"|t:$QUALIFIED_MODULE}:
									</strong></label>
								<div class="col-md-6 ">
							...
							<div class="">
								<label
									class=""><strong>{"LBL_EDIT_PRIVILIGES_FOR_THIS_PROFILE"|t:$QUALIFIED_MODULE}:</strong></label><br>
										...
										<tr class="blockHeader">
											<th width="30%" style="border-left: 1px solid #DDD !important;">
												<input checked="true" class="alignTop" type="checkbox"
													id="mainModulesCheckBox" />&nbsp;
												{"LBL_MODULES"|t:$QUALIFIED_MODULE}
											</th>
											<th data-hide='phone' width="14%" style="border-left: 1px solid #DDD !important;">
												<input {if empty($RECORD_ID) && empty($IS_DUPLICATE_RECORD)} class="alignTop"
													checked="true" {/if} type="checkbox" id="mainAction4CheckBox" />&nbsp;
												{'LBL_VIEW_PRIVILEGE'|t:$QUALIFIED_MODULE}
											</th>
											<th data-hide='phone' width="14%" style="border-left: 1px solid #DDD !important;">
												<input {if empty($RECORD_ID) && empty($IS_DUPLICATE_RECORD)} class="alignTop"
													checked="true" {/if} type="checkbox" id="mainAction7CheckBox" />&nbsp;
												{'LBL_CREATE_PRIVILIGE'|t:$QUALIFIED_MODULE}
											</th>
											<th data-hide='phone' width="14%" style="border-left: 1px solid #DDD !important;">
												<input {if empty($RECORD_ID) && empty($IS_DUPLICATE_RECORD)} class="alignTop"
													checked="true" {/if} type="checkbox" id="mainAction1CheckBox" />&nbsp;
												{'LBL_EDIT_PRIVILIGE'|t:$QUALIFIED_MODULE}
											</th>
											<th data-hide='phone' width="14%" style="border-left: 1px solid #DDD !important;">
												<input checked="true" class="alignTop" type="checkbox"
													id="mainAction2CheckBox" />&nbsp;
												{'LBL_DELETE_PRIVILIGE'|t:$QUALIFIED_MODULE}
											</th>
											<th width="28%" style="border-left: 1px solid #DDD !important;" nowrap="nowrap">
												{'LBL_FIELD_AND_TOOL_PRVILIGES'|t:$QUALIFIED_MODULE}</th>
										</tr>
									</thead>
									<tbody>
										...
										{foreach from=$PROFILE_MODULES key=TABID item=PROFILE_MODULE}
											...
											{if $MODULE_NAME neq 'Events'}
												<tr>
													<td>
														<input class="modulesCheckBox alignTop" type="checkbox"
															name="permissions[{$TABID}][is_permitted]" data-value="{$TABID}"
															data-module-state=""
															{if $RECORD_MODEL->hasModulePermission($PROFILE_MODULE)}checked="true"
															{else} data-module-unchecked="true" 
															{/if}>
														...
														<td style="border-left: 1px solid #DDD !important;">
															...
															{if $ACTION_MODEL->isModuleEnabled($PROFILE_MODULE)}
																<input style="margin-left: 45% !important" class="action{$ACTION_ID}CheckBox"
																	...
																	{elseif empty($RECORD_ID) && empty($IS_DUPLICATE_RECORD)} checked="true"
																	{else} data-action{$ACTION_ID}-unchecked="true" 
																	{/if}>
															{/if}
														</td>
													{/foreach}
													<td style="border-left: 1px solid #DDD !important;">
														{if $PROFILE_MODULE->getFields() || $PROFILE_MODULE->isUtilityActionEnabled()}
															<div class="row">
																...
																<span class="col-md-4"><button type="button" data-handlerfor="fields"
																		data-togglehandler="{$TABID}-fields" class="btn btn-xs btn-default"
												...
												{if $PROFILE_MODULE->getFields()}
													<tr class="hide">
														<td colspan="6" data-toggle-visible="false" class="row"
															style="padding-left: 5%;padding-right: 5%">
															<div class="row" data-togglecontent="{$TABID}-fields">
																<div class="col-md-12">
																	<label class="themeTextColor font-x-large pull-left"><strong>{"LBL_FIELDS"|t:$QUALIFIED_MODULE}{if $MODULE_NAME eq 'Calendar'}
																				{"LBL_OF"|t:$MODULE_NAME}
																			{"LBL_TASKS"|t:$MODULE_NAME}{/if}</strong></label>
																	<div class="pull-right">
																		<span class="mini-slider-control ui-slider" data-value="0">
																			<a style="margin-top: 3px;" class="ui-slider-handle"></a>
																		</span>
																		<span
																			style="margin: 0 20px;">{"LBL_INVISIBLE"|t:$QUALIFIED_MODULE}</span>&nbsp;&nbsp;
																		<span class="mini-slider-control ui-slider" data-value="1">
																			<a style="margin-top: 3px;" class="ui-slider-handle"></a>
																		</span>
																		<span
																			style="margin: 0 20px;">{"LBL_READ_ONLY"|t:$QUALIFIED_MODULE}</span>&nbsp;&nbsp;
																		<span class="mini-slider-control ui-slider" data-value="2">
																			<a style="margin-top: 3px;" class="ui-slider-handle"></a>
																		</span>
																		<span
																			style="margin: 0 20px;">{"LBL_WRITE"|t:$QUALIFIED_MODULE}</span>
																	</div>
																	...
																	{foreach from=$PROFILE_MODULE->getFields() key=FIELD_NAME item=FIELD_MODEL name="fields"}
																		{if $FIELD_MODEL->isActiveField()}
																			...
																			{if $COUNTER % 3 == 0}
																				...
																				{/if}
																				<div class='col-md-4 col-sm-6 col-xs-12 div-bordered padding10'
																					...
																					<input type="hidden"
																						...
																						readonly="true">
																					<div class="mini-slider-control editViewMiniSlider pull-left"
																						data-locked="{$FIELD_LOCKED}" data-range="{$FIELD_ID}"
																						data-value="{$RECORD_MODEL->getModuleFieldPermissionValue($PROFILE_MODULE, $FIELD_MODEL)}">
																					</div>
																					<div class="pull-left">
																						{if $FIELD_MODEL->isMandatory()}<span
																...
																{if $MODULE_NAME eq 'Calendar'}
																	...
																	{assign var=COUNTER value=0}
																	<label class="themeTextColor font-x-large pull-left"><strong>{"LBL_FIELDS"|t:$QUALIFIED_MODULE}
																			{'LBL_OF'|t:$EVENT_MODULE->getName()}
																			{'LBL_EVENTS'|t:$EVENT_MODULE->getName()}</strong></label>
																	<div class="col-xs-12 paddingLRZero marginBottom10px">
																		{foreach from=$EVENT_MODULE->getFields() key=FIELD_NAME item=FIELD_MODEL name="fields"}
																			{if $FIELD_MODEL->isActiveField()}
																				...
																				{if $COUNTER % 3 == 0}
																					<div class='col-md-12 paddingLRZero'>
																					{/if}
																					<div class='col-md-4 col-sm-6 col-xs-12  padding10 div-bordered'
																						style="border-left: 1px solid #DDD !important;">
																						...
																						<div class="mini-slider-control editViewMiniSlider pull-left"
																							data-locked="{$FIELD_LOCKED}" data-range="{$FIELD_ID}"
																							data-value="{$RECORD_MODEL->getModuleFieldPermissionValue($EVENT_MODULE, $FIELD_MODEL)}">
																						</div>
																						<div class="pull-left">
																							{if $FIELD_MODEL->isMandatory()}<span
																								class="redColor">*</span>{/if}
												...
												{if $ALL_UTILITY_ACTIONS_ARRAY}
													<tr class="hide">
														<td colspan="6" data-toggle-visible="false" class="row"
															style="padding-left: 5%;padding-right: 5%;background-image: none !important;">
															<div class="row" data-togglecontent="{$TABID}-fields">
																<div class="col-xs-12 paddingLRZero"><label
																		class="themeTextColor font-x-large pull-left"><strong>{"LBL_TOOLS"|t:$QUALIFIED_MODULE}</strong></label>
																</div>
																<div class="col-md-12 paddingLRZero marginBottom10px">
																	{foreach from=$ALL_UTILITY_ACTIONS_ARRAY item=ACTION_MODEL name="actions"}
																		{if $smarty.foreach.actions.index % 3 == 0}
																			...
																			<div class='col-md-4 col-sm-6 col-xs-12 padding10'
																				{if $smarty.foreach.actions.last && (($smarty.foreach.actions.index+1) % 3 neq 0)}
																					{assign var="index" value=($smarty.foreach.actions.index+1) % 3}
																					{assign var="colspan" value=4-$index} colspan="{$colspan}"
																				{/if}>
