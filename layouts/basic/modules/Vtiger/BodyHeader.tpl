									...
									{foreach key=index item=obj from=$CHILD_LINKS}
										{if $obj->getLabel() eq NULL}
											...
											{else}
												...
												{/if}
											<li>
												<a target="{$obj->target}" id="menubar_item_right_{\App\Modules\Vtiger\Helpers\Util::replaceSpaceWithUnderScores($label)}" {if $label=='Switch to old look'}switchLook{/if} href="{$href}" {$onclick}
