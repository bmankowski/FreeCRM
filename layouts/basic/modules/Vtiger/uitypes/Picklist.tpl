...
-->*}
{strip}
<!-- layouts/basic/modules/Vtiger/uitypes/Picklist.tpl -->
	...
	<select class="chzn-select form-control" title="{$FIELD_MODEL->get('label')|t:$MODULE}" name="{$FIELD_MODEL->getFieldName()}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" data-fieldinfo='{$FIELD_INFO|escape}' {if !empty($SPECIAL_VALIDATOR)}data-validator='{\App\Json::encode($SPECIAL_VALIDATOR)}'{/if} data-selected-value='{$FIELD_MODEL->get('fieldvalue')}' {if $FIELD_MODEL->isEditableReadOnly()}readonly="readonly"{/if}>
		...
		{foreach item=PICKLIST_VALUE key=PICKLIST_NAME from=$PICKLIST_VALUES}
			<option value="{\App\Modules\Vtiger\helpers\Util::toSafeHTML($PICKLIST_NAME)}" title="{$PICKLIST_VALUE}" {if trim(decode_html($FIELD_MODEL->get('fieldvalue'))) eq trim($PICKLIST_NAME)} selected {/if}>{$PICKLIST_VALUE}</option>
		{/foreach}
	</select>
<!--/layouts/basic/modules/Vtiger/uitypes/Picklist.tpl -->
{/strip}
