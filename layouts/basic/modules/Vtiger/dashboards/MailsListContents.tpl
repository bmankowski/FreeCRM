...
<div>
	...
	<div class="row mailRow" data-mailId="{$key}">
		<div class="col-md-12" style="font-size:x-small;">
			...
			<h5 style="margin-left:2%;">{$item->get('subject')} {if $item->get('attachments') && count($item->get('attachments')) > 0}<img alt="{vtranslate('LBL_ATTACHMENT')}" class="pull-right" src="{Yeti_Layout::getLayoutFile('modules/OSSMailView/attachment.png')}" />{/if}<h5>
		</div>
