...
-->*}
{strip}
<!-- layouts/basic/modules/OSSMailView/DetailViewHeaderTitle.tpl -->
	<div class="col-md-12 paddingLRZero row">
		...
		<div class="col-xs-10 margin0px">
			...
			<div class="paddingLeft5px pull-left">
				...
				<span class="muted">
					<small><em>{"Sent"|t:"OSSMailView"}</em></small>
					<span><small><em>&nbsp;{$RECORD->get('createdtime')}</em></small></span>
				</span>
				<div>
					<strong>{"LBL_OWNER"|t} : {\App\Fields\Owner::getLabel($RECORD->get('assigned_user_id'))}</strong>
				</div>
			</div>
		</div>
