					...
					{* Fix for empty Recycle bin Button *} 
                                        {foreach item=LISTVIEW_BASICACTION from=$LISTVIEW_LINKS['LISTVIEWBASIC']} 
                                                <span class="btn-group">  
                                                    <button id="{$MODULE}_listView_basicAction_{\App\Modules\Base\Helpers\Util::replaceSpaceWithUnderScores($LISTVIEW_BASICACTION->getLabel())}" class="btn {if $LISTVIEW_BASICACTION->getClassName() neq ''}{if $LISTVIEW_BASICACTION->getClassName()|strrpos:"btn-" === false}btn-default {/if}{$LISTVIEW_BASICACTION->getClassName()}{else}btn-default{/if} {if $LISTVIEW_BASICACTION->getLabel() eq 'LBL_EMPTY_RECYCLEBIN'}clearRecycleBin{/if}" {if stripos($LISTVIEW_BASICACTION->getUrl(), 'javascript:')===0} onclick='{$LISTVIEW_BASICACTION->getUrl()|substr:strlen("javascript:")};'{else} onclick='window.location.href="{$LISTVIEW_BASICACTION->getUrl()}"'{/if} {if $LISTVIEW_BASICACTION->getLabel() eq 'LBL_EMPTY_RECYCLEBIN' && $DELETED_RECORDS_TOTAL_COUNT eq 0} disabled="disabled" {/if}>{if $LISTVIEW_BASICACTION->getIcon() neq ''}<span class="{$LISTVIEW_BASICACTION->getIcon()}"></span>&nbsp;{/if}<strong>{$LISTVIEW_BASICACTION->getLabel()|t:$MODULE}</strong></button> 
                                                </span> 
                                        {/foreach} 
				</span>
			<span class="btn-toolbar col-md-4">
