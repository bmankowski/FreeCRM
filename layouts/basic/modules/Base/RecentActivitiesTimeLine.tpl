...
{strip}
<!-- layouts/basic/modules/Base/RecentActivitiesTimeLine.tpl -->
							...
							<li>
									...
									<div class="timeline-item{if $NEW_CHANGE} bgWarning{/if}">
										...
										<div class="timeline-body row no-margin">
											<span class="time pull-right">
												<span title="{$RECENT_ACTIVITY->getDisplayActivityTime()}">{\App\Modules\Base\Helpers\Util::formatDateDiffInStrings($RECENT_ACTIVITY->getParent()->get('createdtime'))}</span>
											</span>
															...
															{if $FIELDMODEL->get('postvalue') neq ''}
																<strong class="moreContent">
																	<span class="teaserContent">
																		{\App\Modules\Base\Helpers\Util::toVtiger6SafeHTML($FIELDMODEL->getNewValue())}
																	</span>
								...
								{else if $RECENT_ACTIVITY->isUpdate()}
									...
									<div class="timeline-item{if $NEW_CHANGE} bgWarning{/if}">
										...
										<div class="timeline-body row no-margin">
											<span class="time pull-right">
												<span title="{$RECENT_ACTIVITY->getDisplayActivityTime()}">{\App\Modules\Base\Helpers\Util::formatDateDiffInStrings($RECENT_ACTIVITY->getActivityTime())}</span>
											</span>
												...
												{if $FIELDMODEL && $FIELDMODEL->getFieldInstance() && $FIELDMODEL->getFieldInstance()->isViewable() && $FIELDMODEL->getFieldInstance()->getDisplayType() neq '5'}
													<div class='font-x-small updateInfoContainer'>
														...
														{if $FIELDMODEL->get('prevalue') neq '' && $FIELDMODEL->get('postvalue') neq '' && !($FIELDMODEL->getFieldInstance()->getFieldDataType() eq 'reference' && ($FIELDMODEL->get('postvalue') eq '0' || $FIELDMODEL->get('prevalue') eq '0'))}
															...
															<strong class="moreContent">
																<span class="teaserContent">
																	{\App\Modules\Base\Helpers\Util::toVtiger6SafeHTML($FIELDMODEL->getOldValue())}
																</span>
														...
														{else if $FIELDMODEL->get('postvalue') eq '' || ($FIELDMODEL->getFieldInstance()->getFieldDataType() eq 'reference' && $FIELDMODEL->get('postvalue') eq '0')}
															...
															( <del>{\App\Modules\Base\Helpers\Util::toVtiger6SafeHTML($FIELDMODEL->getOldValue())}</del> )
														{else}
															&nbsp;{"LBL_CHANGED"|t}
														{/if}
										...
										<div class="timeline-body row no-margin">
											...
											<span>
												{if \App\Modules\Users\Models\Privileges::isPermitted($RELATION->getLinkedRecord()->getModuleName(), 'DetailView', $RELATION->getLinkedRecord()->getId())}
													<strong class="moreContent">
														<span class="teaserContent">
															{\App\Modules\Base\Helpers\Util::toVtiger6SafeHTML($RELATION->getValue())}
														</span>
