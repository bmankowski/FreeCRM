...
-->*}
{strip}
<!-- layouts/basic/modules/Base/uitypes/Owner.tpl -->
	...
	{if $FIELD_MODEL->get('uitype') eq '53'}
		...
		<select class="select2 form-control {$ASSIGNED_USER_ID}" title="{$FIELD_MODEL->get('label')|t:$MODULE}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" data-name="{$ASSIGNED_USER_ID}" name="{$ASSIGNED_USER_ID}" data-fieldinfo='{$FIELD_INFO}' {if !empty($SPECIAL_VALIDATOR)}data-validator={\App\Utils\Json::encode($SPECIAL_VALIDATOR)}{/if} {if $FIELD_MODEL->isEditableReadOnly()}readonly="readonly"{/if} {if $USER_MODEL->isAdminUser() == false && $ROLE_RECORD_MODEL->get('changeowner') == 0}readonly="readonly"{/if} 
			...
			{if !AppConfig::performance('SEARCH_OWNERS_BY_AJAX')}
				{if $VIEW eq 'MassEdit'}<option value="">{"LBL_SELECT_OPTION"|t:"Vtiger"}</option>{/if}
