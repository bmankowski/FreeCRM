...
-->*}
{strip}
<!-- layouts/basic/modules/Base/uitypes/MultiPicklist.tpl -->
...
<select id="{$MODULE}_{$VIEW}_fieldName_{$FIELD_MODEL->get('name')}" title="{$FIELD_MODEL->get('label')|t:$MODULE}" multiple class="chzn-select form-control col-md-12 {if !empty($NOT_DISPLAY_LIST_VALUES)} hideSelected{/if}" name="{$FIELD_MODEL->getFieldName()}[]" data-fieldinfo='{$FIELD_INFO}' {if $FIELD_MODEL->isMandatory() eq true} data-validation-engine="validate[required,funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" {if !empty($SPECIAL_VALIDATOR)}data-validator='{\App\Json::encode($SPECIAL_VALIDATOR)}'{/if} {/if} {if $FIELD_MODEL->isEditableReadOnly()}readonly="readonly"{/if}>
    {foreach item=PICKLIST_VALUE key=PICKLIST_NAME from=$PICKLIST_VALUES}
		<option value="{\App\Modules\Base\Helpers\Util::toSafeHTML($PICKLIST_NAME)}" {if in_array(\App\Modules\Base\Helpers\Util::toSafeHTML($PICKLIST_NAME), $FIELD_VALUE_LIST)} selected {/if}{if $NOT_DISPLAY_LIST_VALUES && array_key_exists($PICKLIST_NAME, $NOT_DISPLAY_LIST_VALUES)} class="hide" {/if}>{$PICKLIST_VALUE}</option>
    {/foreach}
	{foreach from=$NOT_DISPLAY_LIST_VALUES key=PICKLIST_NAME item=ITERATION}
		<option value="{\App\Modules\Base\Helpers\Util::toSafeHTML($PICKLIST_NAME)}" {if in_array(\App\Modules\Base\Helpers\Util::toSafeHTML($PICKLIST_NAME), $FIELD_VALUE_LIST)} selected {/if} class="hide">{$PICKLIST_NAME}</option>
	{/foreach}
</select>
<!--/layouts/basic/modules/Base/uitypes/MultiPicklist.tpl -->
{/strip}
