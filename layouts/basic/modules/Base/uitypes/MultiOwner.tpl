...
-->*}
{strip}
<!-- layouts/basic/modules/Base/uitypes/MultiOwner.tpl -->
	...
	{if $FIELD_MODEL->get('uitype') eq '54'}
		...
		<select class="chzn-select form-control {$ASSIGNED_USER_ID}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" data-name="{$ASSIGNED_USER_ID}" name="{$ASSIGNED_USER_ID}[]" data-fieldinfo='{$FIELD_INFO}' {if !empty($SPECIAL_VALIDATOR)}data-validator={\App\Utils\Json::encode($SPECIAL_VALIDATOR)}{/if} multiple>
