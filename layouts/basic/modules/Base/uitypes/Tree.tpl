...
{strip}
<!-- layouts/basic/modules/Base/uitypes/Tree.tpl -->
...
{assign var="ALL_VALUE" value=\App\Modules\Base\Helpers\Util::toSafeHTML(\App\Utils\Json::encode($FIELD_MODEL->getUITypeModel()->getAllValue()))}
...
<div class="input-group">
	...
	<input id="{$FIELD_NAME}_display" name="{$FIELD_MODEL->getFieldName()}_display" type="text" class="{if (($VIEW eq 'Edit'))} col-md-7 {else} col-md-8 {/if}	marginLeftZero treeAutoComplete form-control" {if !empty($displayId)}readonly="true"{/if}
	 ...
	 data-fieldinfo='{$FIELD_INFO}' {if $FIELD_MODEL->get('displaytype') != 10}placeholder="{"LBL_TYPE_SEARCH"|t:$MODULE}"{/if}
