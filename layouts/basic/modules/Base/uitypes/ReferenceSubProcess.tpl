...
{strip}
<!-- layouts/basic/modules/Base/uitypes/ReferenceSubProcess.tpl -->
	...
	{if {$REFERENCE_LIST_COUNT} gt 1}
		...
		{/if}
	{/if}
	{assign var=REFERENCE_MODULE_MODEL value=\App\Modules\Base\Models\Module::getInstance($REFERENCE_LIST[0])}
	...
	<div class="input-group referenceGroup">
			...
			<div class="input-group-addon noSpaces referenceModulesListGroup">
				<select id="{$MODULE}_editView_fieldName_{$FIELD_MODEL->getName()}_dropDown" class="referenceModulesList" title="{"LBL_RELATED_MODULE_TYPE"|t}" required="required">
					{foreach key=index item=REFERENCE from=$REFERENCE_LIST}
						{assign var=REFERENCE_MODULE_MODEL value=\App\Modules\Base\Models\Module::getInstance($REFERENCE)}
		...
		{/if}
		<input id="{$FIELD_NAME}_display" name="{$FIELD_MODEL->getFieldName()}_display" type="text" title="{\App\Modules\Base\Helpers\Util::toSafeHTML($FIELD_MODEL->getEditViewDisplayValue($DISPLAYID))}" class="marginLeftZero form-control autoComplete" {if !empty($DISPLAYID)}readonly="true"{/if}
			   value="{\App\Modules\Base\Helpers\Util::toSafeHTML($FIELD_MODEL->getEditViewDisplayValue($DISPLAYID))}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]"
			   ...
			   {if !empty($SPECIAL_VALIDATOR)}data-validator='{\App\Utils\Json::encode($SPECIAL_VALIDATOR)}'{/if} {if $FIELD_MODEL->isEditableReadOnly()}readonly="readonly"{/if}/>
		<span class="input-group-btn cursorPointer">
			...
			<button class="btn btn-default relatedPopup" type="button" {if $REFERENCE_MODULE_MODEL == false || $FIELD_MODEL->isEditableReadOnly()}disabled{/if}>
				<span id="{$MODULE}_editView_fieldName_{$FIELD_NAME}_select" class="glyphicon glyphicon-search" title="{"LBL_SELECT"|t:$MODULE}" ></span>
			</button>
