{strip}
<!-- layouts/basic/modules/Base/menu/Module.tpl -->
{*<!-- {[The file is published on the basis of YetiForce Public License that can be found in the following directory: licenses/License.html]} --!>*}
	{if (isset($ACTIVE_MODULES[$MENU.mod]) && $ACTIVE_MODULES[$MENU.mod]) AND ($PRIVILEGESMODEL->isAdminUser() || $PRIVILEGESMODEL->hasGlobalReadPermission() || $PRIVILEGESMODEL->hasModulePermission($MENU['tabid']) ) }
		{assign var=ICON value=\App\Modules\Base\Models\Menu::getMenuIcon($MENU, $MENU['name']|t:$MENU['mod'])}
		<li class="menuModule moduleColor_{$MENU['mod']} {if !$HASCHILDS}hasParentMenu{/if}" data-id="{$MENU['id']}" role="menuitem" tabindex="{$TABINDEX}" {if $HASCHILDS}aria-haspopup="{$HASCHILDS}"{/if}>
			<a class="{if $MENU['name'] == $MODULE}active{/if} {if $ICON}hasIcon{/if} {if isset($MENU['hotkey'])}hotKey{/if}" {if isset($MENU['hotkey'])}data-hotkeys="{$MENU['hotkey']}"{/if} href="{$MENU['dataurl']}" 
				...
				{$ICON}
				<span class="menuName">{$MENU['name']|t:$MENU['mod']}</span>
			</a>
			{if $DEVICE == 'Desktop'}
