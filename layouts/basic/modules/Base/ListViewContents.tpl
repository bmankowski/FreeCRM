...
{strip}
	...
	<div id="listViewContents" class="listViewContents">
		...
		<div id="selectAllMsgDiv" class="alert-block msgDiv noprint">
			<strong><a id="selectAllMsg">{"LBL_SELECT_ALL"|t:$MODULE}&nbsp;{$MODULE|t:$MODULE}&nbsp;(<span
				...
				<table class="table table-bordered listViewEntriesTable {$WIDTHTYPE}">
					<thead>
						<tr class="listViewHeaders">
							...
							{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
								<th {if !empty($LISTVIEW_HEADER->get('maxwidthcolumn'))}style="width:{$LISTVIEW_HEADER->get('maxwidthcolumn')}%"
										{/if} {if $LISTVIEW_HEADER@last}colspan="2" {/if}
										class="noWrap {if $COLUMN_NAME eq $LISTVIEW_HEADER->get('column')}columnSorted{/if}">
										<a href="javascript:void(0);" class="listViewHeaderValues pull-left"
										...
										{/if}
										data-columnname="{$LISTVIEW_HEADER->get('column')}">{$LISTVIEW_HEADER->get('label')|t:$MODULE}
										&nbsp;&nbsp;{if $COLUMN_NAME eq $LISTVIEW_HEADER->get('column')}<span
									...
									{if $LISTVIEW_HEADER->getFieldDataType() eq 'tree' || $LISTVIEW_HEADER->getFieldDataType() eq 'categoryMultipicklist'}
										...
										<div class='pull-left'>
											<span class="pull-right popoverTooltip delay0" data-placement="top"
												data-original-title="{$LISTVIEW_HEADER->get('label')|t:$MODULE}"
					...
					{foreach item=LISTVIEW_ENTRY from=$LISTVIEW_ENTRIES name=listview}
						...
						<tr class="listViewEntries" data-id='{$LISTVIEW_ENTRY->getId()}'
							...
							{if $LISTVIEW_ENTRY->colorList}style="background-color: {$LISTVIEW_ENTRY->colorList['background']};color: {$LISTVIEW_ENTRY->colorList['text']};"
							{/if}>
							...
							{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS name=listHeaderForeach}
								...
								<td class="listViewEntryValue noWrap {$WIDTHTYPE}"
									...
									data-raw-value="{\App\Modules\Base\Helpers\Util::toSafeHTML($LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME))}">
				...
				{if $LISTVIEW_ENTRIES_COUNT eq '0'}
					<table class="emptyRecordsDiv">
						<tbody>
							<tr>
								<td>
									{"LBL_RECORDS_NO_FOUND"|t}.{if $IS_MODULE_EDITABLE} <a
										href="{$MODULE_MODEL->getCreateRecordUrl()}">{"LBL_CREATE_SINGLE_RECORD"|t}</a>{/if}
								</td>
							</tr>
						</tbody>
					</table>
