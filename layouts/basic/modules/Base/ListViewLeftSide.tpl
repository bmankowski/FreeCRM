...
{strip}
<!-- layouts/basic/modules/Base/ListViewLeftSide.tpl -->
	<div>
	...
	{/if}
	<div>
		{if in_array($MODULE, \App\Core\AppConfig::module('ModTracker', 'SHOW_TIMELINE_IN_LISTVIEW')) && $MODULE_MODEL->isPermitted('TimeLineList')}
			<a type="button" data-url="{$LISTVIEW_ENTRY->getTimeLineUrl()}" class="timeLineIconList hide">
				<span class="glyphicon" aria-hidden="true"></span>
			</a>
		{/if}
		{if \App\Core\AppConfig::module('ModTracker', 'UNREVIEWED_COUNT') && $MODULE_MODEL->isPermitted('ReviewingUpdates') && $MODULE_MODEL->isTrackingEnabled() && $LISTVIEW_ENTRY->isViewable()}
