...
-->*}
{strip}
<!-- layouts/basic/modules/Base/SummaryViewContents.tpl -->
			...
			<tr class="summaryViewEntries">
				...
				<td class="fieldValue {$WIDTHTYPE}" style="width:65%">
                    <div class="row">
                        ...
                        {if !$IS_READ_ONLY && $FIELD_MODEL->isEditable() eq 'true' && ($FIELD_MODEL->getFieldDataType()!=\App\Modules\Base\Models\Field::REFERENCE_TYPE) && $IS_AJAX_ENABLED && $FIELD_MODEL->isAjaxEditable() eq 'true' && $FIELD_MODEL->get('uitype') neq 69}
...
<div class="row">
	<div class="col-md-4 toggleViewByMode">
		{if !$IS_READ_ONLY}
			...
			{assign var="CURRENT_MODE_LABEL" value="{'LBL_COMPLETE_DETAILS'|t:{$MODULE_NAME}}"}
			<button type="button" class="btn btn-default btn-block changeDetailViewMode cursorPointer"><strong>{"LBL_SHOW_FULL_DETAILS"|t:$MODULE_NAME}</strong></button>
