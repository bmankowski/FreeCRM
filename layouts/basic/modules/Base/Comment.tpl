				...
				<div class="editedStatus visible-lg-block"  name="editStatus">
					<div class="col-xs-6">
						<span class="{if empty($REASON_TO_EDIT)}hide{/if} col-xs-6 editReason">
							<p><small>[ {"LBL_EDIT_REASON"|t:$MODULE_NAME} ] : <span  name="editReason" class="textOverflowEllipsis">{nl2br($REASON_TO_EDIT)}</span></small></p>
						</span>
						{if $COMMENT->getCommentedTime() neq $COMMENT->getModifiedTime()}
							<span class="{if !empty($REASON_TO_EDIT)} col-xs-6{/if}">
								<span class="pull-right">
									<p class="muted"><small><em>{"LBL_MODIFIED"|t:$MODULE_NAME}</em></small>&nbsp;<small title="{\App\Modules\Base\Helpers\Util::formatDateTimeIntoDayString($COMMENT->getModifiedTime())}" class="commentModifiedTime">{\App\Modules\Base\Helpers\Util::formatDateDiffInStrings($COMMENT->getModifiedTime())}</small></p>
								</span>
							</span>
						{/if}
					</div>
						...
						{if $CHILD_COMMENTS_MODEL neq null and ($CHILDS_ROOT_PARENT_ID neq $PARENT_COMMENT_ID)}
							<span class="viewThreadBlock" data-child-comments-count="{$CHILD_COMMENTS_COUNT}">
								<button type="button" class="btn btn-xs btn-info viewThread marginLeft5">
									<span class="childCommentsCount">{$CHILD_COMMENTS_COUNT}</span>&nbsp;{if $CHILD_COMMENTS_COUNT eq 1}{"LBL_REPLY"|t:$MODULE_NAME}{else}{"LBL_REPLIES"|t:$MODULE_NAME}{/if}&nbsp;
