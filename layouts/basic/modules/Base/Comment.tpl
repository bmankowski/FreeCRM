				...
				<div class="commentTitle row no-margin" id="{$COMMENT->getId()}">
					...
					<div class="inner">
						<span class="pull-right paddingRight15">
							<p class="muted"><small
									title="{\App\Modules\Base\Helpers\Util::formatDateTimeIntoDayString($COMMENT->getCommentedTime())}">{\App\Modules\Base\Helpers\Util::formatDateDiffInStrings($COMMENT->getCommentedTime())}</small>
							</p>
						</span>
						...
						{if $COMMENT->getCommentedTime() neq $COMMENT->getModifiedTime()}
							<span class="{if !empty($REASON_TO_EDIT)} col-xs-6{/if}">
								<span class="pull-right">
									<p class="muted"><small><em>{"LBL_MODIFIED"|t:$MODULE_NAME}</em></small>&nbsp;<small
											title="{\App\Modules\Base\Helpers\Util::formatDateTimeIntoDayString($COMMENT->getModifiedTime())}"
											class="commentModifiedTime">{\App\Modules\Base\Helpers\Util::formatDateDiffInStrings($COMMENT->getModifiedTime())}</small>
									</p>
								</span>
							</span>
						{/if}
						...
						{if \App\Modules\Users\Models\Privileges::isPermitted('ModComments','EditableComments') && $CURRENTUSER->getId() eq $COMMENT->get('userid')}
							<button type="button" class="btn btn-xs btn-primary editComment feedback marginLeft5">
								<span class="glyphicon glyphicon-pencil"
									aria-hidden="true"></span>&nbsp;{"LBL_EDIT"|t:$MODULE_NAME}
							</button>
						{/if}
						{if $COMMENTS_MODULE_MODEL->isPermitted('Delete')}
							<button type="button" class="btn btn-xs btn-danger deleteComment marginLeft5">
								<span class="glyphicon glyphicon-remove"
									aria-hidden="true"></span>&nbsp;{"LBL_DELETE"|t:$MODULE_NAME}
							</button>
						{/if}
						...
						{if $CHILD_COMMENTS_MODEL neq null and ($CHILDS_ROOT_PARENT_ID neq $PARENT_COMMENT_ID)}
							<span class="viewThreadBlock" data-child-comments-count="{$CHILD_COMMENTS_COUNT}">
								<button type="button" class="btn btn-xs btn-info viewThread marginLeft5">
									<span
										class="childCommentsCount">{$CHILD_COMMENTS_COUNT}</span>&nbsp;{if $CHILD_COMMENTS_COUNT eq 1}{"LBL_REPLY"|t:$MODULE_NAME}{else}{"LBL_REPLIES"|t:$MODULE_NAME}{/if}&nbsp;
