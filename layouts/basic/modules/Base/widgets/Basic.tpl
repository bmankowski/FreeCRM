{strip}
	<!-- layouts/basic/modules/Base/widgets/Basic.tpl -->
	<div class="summaryWidgetContainer">
		<div class="widgetContainer_{$key} widgetContentBlock"
			data-url="{\App\Modules\Base\Helpers\Util::toSafeHTML($WIDGET['url'])}" data-name="{$WIDGET['label']}"
				...
				<div class="row">
					...
					{if isset($WIDGET['switchHeader'])}
						<div class="col-xs-8 col-md-4 col-sm-3 paddingBottom10">
							<input class="switchBtn switchBtnReload filterField" type="checkbox" checked="" data-size="small"
								data-label-width="5" data-on-text="{$WIDGET['switchHeaderLables']['on']}"
								data-off-text="{$WIDGET['switchHeaderLables']['off']}" data-urlparams="search_params"
								data-on-val='{$WIDGET['switchHeader']['on']}' data-off-val='{$WIDGET['switchHeader']['off']}'>
						</div>
					{/if}
						...
						<div class="pull-right">
							<div class="btn-group">
								{if isset($WIDGET['data']['actionSelect']) || isset($WIDGET['data']['action'])}
									{assign var=VRM value=\App\Modules\Base\Models\Record::getInstanceById($RECORD->getId(), $MODULE_NAME)}
									{assign var=VRMM value=\App\Modules\Base\Models\RelationListView::getInstance($VRM, $WIDGET['data']['relatedmodule'])}
									...
									{/if}
									{if isset($WIDGET['data']['action']) && $WIDGET['data']['action'] eq 1}
				...
				<div class="row">
						...
						<div class="{$span} form-group-sm">
							...
							<select class="select2 filterField form-control input-sm" name="{$FIELD_MODEL->get('name')}"
								...
								{foreach item=PICKLIST_VALUE key=PICKLIST_NAME from=$PICKLIST_VALUES}
									<option value="{$PICKLIST_NAME}" {if $FIELD_MODEL->get('fieldvalue') eq $PICKLIST_NAME} selected
					...
					{if isset($WIDGET['data']['checkbox']) && $WIDGET['data']['checkbox'] neq '-'}
						<div class="{$span} small-select">
							...
							<div class="pull-right">
								<input class="switchBtn switchBtnReload filterField" type="checkbox" checked="" data-size="mini"
									data-label-width="5" data-on-text="{$WIDGET['checkboxLables']['on']}"
									data-off-text="{$WIDGET['checkboxLables']['off']}" data-urlparams="search_params"
									data-on-val='{$WIDGET['checkbox']['on']}' data-off-val='{$WIDGET['checkbox']['off']}'>
							</div>
						</div>
					{/if}
				</div>
