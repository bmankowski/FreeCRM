							...
							{foreach item=HEADER_FIELD from=$RELATED_HEADERS}
								...
								<td class="{$WIDTHTYPE}" data-field-type="{$HEADER_FIELD->getFieldDataType()}" nowrap>
									...
									{elseif $RELATED_HEADERNAME eq 'listprice' || $RELATED_HEADERNAME eq 'unit_price'}
										{\App\Fields\CurrencyField::convertToUserFormat($RELATED_RECORD->get($RELATED_HEADERNAME), null, true)}
										{if $RELATED_HEADERNAME eq 'listprice'}
											{assign var="LISTPRICE" value=\App\Fields\CurrencyField::convertToUserFormat($RELATED_RECORD->get($RELATED_HEADERNAME), null, true)}
										{/if}
									{else}
