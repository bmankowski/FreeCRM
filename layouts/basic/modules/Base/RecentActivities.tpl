...
-->*}
{strip}
<!-- layouts/basic/modules/Base/RecentActivities.tpl -->
												...
												{if $FIELDMODEL && $FIELDMODEL->getFieldInstance() && $FIELDMODEL->getFieldInstance()->isViewable() && $FIELDMODEL->getFieldInstance()->getDisplayType() neq '5'}
													<div class="font-x-small updateInfoContainer">
														...
														{if $FIELDMODEL->get('prevalue') neq '' && $FIELDMODEL->get('postvalue') neq '' && !($FIELDMODEL->getFieldInstance()->getFieldDataType() eq 'reference' && ($FIELDMODEL->get('postvalue') eq '0' || $FIELDMODEL->get('prevalue') eq '0'))}
															...
															{\App\Modules\Base\Helpers\Util::toVtiger6SafeHTML($FIELDMODEL->getDisplayValue(decode_html($FIELDMODEL->get('prevalue'))))|t:$MODULE_NAME}</strong>
														{else if $FIELDMODEL->get('postvalue') eq '' || ($FIELDMODEL->getFieldInstance()->getFieldDataType() eq 'reference' && $FIELDMODEL->get('postvalue') eq '0')}
															&nbsp; <strong> {"LBL_DELETED"|t} </strong> ( <del>{\App\Modules\Base\Helpers\Util::toVtiger6SafeHTML($FIELDMODEL->getDisplayValue(decode_html($FIELDMODEL->get('prevalue'))))}</del> )
														{else}
															&nbsp;{"LBL_CHANGED"|t}
														{/if}
														{if $FIELDMODEL->get('postvalue') neq '' && !($FIELDMODEL->getFieldInstance()->getFieldDataType() eq 'reference' && $FIELDMODEL->get('postvalue') eq '0')}
															...
															{\App\Modules\Base\Helpers\Util::toVtiger6SafeHTML($FIELDMODEL->getDisplayValue(decode_html($FIELDMODEL->get('postvalue'))))|t:$MODULE_NAME}</strong>
														{/if}
													</div>
												{/if}
											{/foreach}
										</span>
										<span class="pull-right"><p class="muted"><small title="{\App\Modules\Base\Helpers\Util::formatDateTimeIntoDayString($RECENT_ACTIVITY->getParent()->get('createdtime'))}">{\App\Modules\Base\Helpers\Util::formatDateDiffInStrings($RECENT_ACTIVITY->getParent()->get('createdtime'))}</small></p></span>
									</div>
								</li>
							{else if $RECENT_ACTIVITY->isUpdate()}
								<li>
									<div>
										...
										<span class="pull-right"><p class="muted"><small title="{\App\Modules\Base\Helpers\Util::formatDateTimeIntoDayString($RECENT_ACTIVITY->getActivityTime())}">{\App\Modules\Base\Helpers\Util::formatDateDiffInStrings($RECENT_ACTIVITY->getActivityTime())}</small></p></span>
									</div>
									{foreach item=FIELDMODEL from=$RECENT_ACTIVITY->getFieldInstances()}
										{if $FIELDMODEL && $FIELDMODEL->getFieldInstance() && $FIELDMODEL->getFieldInstance()->isViewable() && $FIELDMODEL->getFieldInstance()->getDisplayType() neq '5'}
											<div class='font-x-small updateInfoContainer'>
									...
									<div>
										...
										<span>
												...
												{if $RELATION->getLinkedRecord()->getModuleName() eq 'Calendar'}
													{if \App\Security\Privilege::isPermitted('Calendar', 'DetailView', $RELATION->getLinkedRecord()->getId())} <strong>{$RELATION->getLinkedRecord()->getName()}</strong> {else} {/if}
									...
									<div>
										...
										<span class="pull-right">
											<p class="muted no-margin">
												<small title="{\App\Modules\Base\Helpers\Util::formatDateTimeIntoDayString($RECENT_ACTIVITY->getActivityTime())}">{\App\Modules\Base\Helpers\Util::formatDateDiffInStrings($RECENT_ACTIVITY->getActivityTime())}
												</small>
											</p>
										</span>
									</div>
