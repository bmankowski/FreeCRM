...
-->*}
{strip}
<!-- layouts/basic/modules/Base/UnifiedSearchResults.tpl -->
	...
	<div class="modal-dialog">
        <div class="modal-content">
				...
				{foreach key=module item=searchRecords from=$MATCHING_RECORDS name=matchingRecords}
					{assign var="modulesCount" value=count($searchRecords)}
					<form method="POST" action="index.php?module={$module}&view=ListView" name="form_{$module}"  enctype="multipart/form-data">
						<input type="hidden" id="recordList" name="searchResult" value="{\App\Utils\Json::encode(array_keys($searchRecords))}" />
						<div class="clearfix">
							...
							{if $recordObject->get('permitted')}
								<li id="{$ID}">
									<a target="_blank" id="{$ID}_link" class="cursorPointer" {if stripos($DETAILVIEW_URL, 'javascript:')===0} 
										...
										<span id="{$ID}_time" class="pull-right" title="{\App\Modules\Base\Helpers\Util::formatDateTimeIntoDayString($recordObject->get('createdtime'))}">{\App\Modules\Base\Helpers\Util::formatDateDiffInStrings($recordObject->get('createdtime'))}</span>
									</a>
								</li>
							{else}
								<li id="{$ID}">
