...
<!-- layouts/basic/modules/Base/PopupSearchActions.tpl -->
	...
	{if $SWITCH && !empty($LISTVIEW_ENTRIES)}
		<div class="col-md-4 form-group pull-left">
			<div class="btn-group">
				<input class="switchPopup switchBtn" type="checkbox"{if $RELATED_PARENT_ID} checked{else} disabled{/if} title="{"LBL_POPUP_SWITCH_BUTTON"|t:$MODULE}" data-size="normal" data-label-width="5" data-on-text="{$POPUP_SWITCH_ON_TEXT|default:''}" data-off-text="{"LBL_ALL"|t:$MODULE}" data-on-val="{$RELATED_PARENT_ID|default:''}" data-off-val="0" data-field="relatedParentId">
			</div>
			<div class="btn-group">
