					...
					<ul class="dropdown-menu pull-right">
						{foreach item=CHILDLINK from=$CHILDLINKS}
							...
							{else}
								...
								{/if}
								<li>
									<a target="{$CHILDLINK->target}" id="menubar_item_right_{\App\Modules\Base\Helpers\Util::replaceSpaceWithUnderScores($LABEL)}" {if $LABEL=='Switch to old look'}switchLook{/if} href="{$HREF}" {$ONCLICK}>
										{$LABEL|t:$MODULE}
									</a>
								</li>
							{/if}
				...
				{foreach key=index item=obj from=$MENU_HEADER_LINKS name="compactIndex"}
					...
					{if $smarty.foreach.compactIndex.index neq 0}
						...
						{foreach key=index item=obj from=$childLinks}
							...
							{/if}
						<li>
							<a target="{$obj->target}" id="menubar_item_right_{\App\Modules\Base\Helpers\Util::replaceSpaceWithUnderScores($label)}" {if $label=='Switch to old look'}switchLook{/if} href="{$href}" {$onclick}>{$label|t:$MODULE}</a>
						</li>
					{/foreach}
				{/foreach}
			</ul>
