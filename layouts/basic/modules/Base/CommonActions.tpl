...
-->*}
{strip}
<!-- layouts/basic/modules/Base/CommonActions.tpl -->
    {assign var='count' value=0}
    {assign var="dateFormat" value=$USER_MODEL->get('date_format')}
	<div class="navbar-form navbar-right">
		<div class="dropdown quickActions historyBtn">
			<a data-placement="left" data-toggle="dropdown" class="showHistoryBtn" aria-expanded="false" href="#"><img class='alignMiddle popoverTooltip' src="{vimage_path('history.png')}" alt="{"LBL_PAGES_HISTORY"|t:$MODULE}" class="dropdown-toggle" data-content="{"LBL_PAGES_HISTORY"|t}" /></a>
		</div>	
	</div>
	<div class="navbar-form navbar-right">
		<div class="dropdown quickActions">
			<a class="dropdown-toggle" data-toggle="dropdown" href="#"><img id="menubar_quickCreate" src="{vimage_path('plus.png')}" class="alignMiddle" alt="{"LBL_QUICK_CREATE"|t:$MODULE}" title="{"LBL_QUICK_CREATE"|t:$MODULE}" /></a>
	...
	<div class="select-search navbar-form navbar-left " style="width: 216px;">
		<select class="chzn-select col-md-5" title="{"LBL_SEARCH_MODULE"|t:$MODULE_NAME}" id="basicSearchModulesList" >
			<option value="" class="globalSearch_module_All">{"LBL_ALL_RECORDS"|t:$MODULE_NAME}</option>
			{foreach key=MODULE_NAME item=fieldObject from=$SEARCHABLE_MODULES}
				{if isset($SEARCHED_MODULE) && $SEARCHED_MODULE eq $MODULE_NAME && $SEARCHED_MODULE !== 'All'}
					<option value="{$MODULE_NAME}" class="globalSearch_module_{$MODULE_NAME}" selected>{$MODULE_NAME|t:$MODULE_NAME}</option>
				{else}
					<option value="{$MODULE_NAME}" class="globalSearch_module_{$MODULE_NAME}">{$MODULE_NAME|t:$MODULE_NAME}</option>
				{/if}
			{/foreach}
		</select>
	</div>
			...
			{if $ROLE_DETAIL->get('globalsearchadv') == 1}
				<div class="pull-left">
					<span class="adv-search navbar-form">
						<button class="alignMiddle btn btn-info" id="globalSearch">{"LBL_ADVANCE_SEARCH"|t}</button>
					</span>
				</div>
			{/if}
		</div>
