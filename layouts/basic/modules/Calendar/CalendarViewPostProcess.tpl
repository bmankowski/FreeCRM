		...
		<div class="siteBarContent paddingTop10">
							...
							{foreach item=FILTER key=index from=$CALENDAR_FILTERS->getFilters()}
								{if $FILTER->type == 'checkbox'}
									<div class="checkbox margin0px">
										<label>
											<input type="checkbox" value="{$FILTER->value}" id="filterField_{$FILTER->name}" title="{$FILTER->name}" data-search="{\App\Modules\Base\Helpers\Util::toSafeHTML($FILTER->searchParams)}" class="filterField">{$FILTER->name|t:$MODULE}
										</label>
									</div>
								{/if}
							{/foreach}
			...
			{foreach item=SIDEBARWIDGET key=index from=$QUICK_LINKS['SIDEBARWIDGETRIGHT']}
				<div class="panel panel-primary quickWidget">
					...
					<div class="widgetContainer panel-collapse" id="{$MODULE}_sideBar_{\App\Modules\Base\Helpers\Util::replaceSpaceWithUnderScores($SIDEBARWIDGET->getLabel())}" data-url="{$SIDEBARWIDGET->getUrl()}">
