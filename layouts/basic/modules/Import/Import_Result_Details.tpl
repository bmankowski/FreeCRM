...
<table cellpadding="5" cellspacing="0" align="center" width="100%" class="dvtSelectedCell thickBorder importContents">
		...
		<tr>
			...
			{if $IMPORT_RESULT['SKIPPED'] neq '0'}
				&nbsp;&nbsp;<a class="cursorPointer" 
					onclick="return window.open('index.php?module={$MODULE}&view=ListView&mode=getImportDetails&type=skipped&start=1&foruser={$OWNER_ID}&forModule={$FOR_MODULE}','skipped','width=700,height=650,resizable=no,scrollbars=yes,top=150,left=200');">
				{'LBL_DETAILS'|t:$MODULE}</a>
			{/if}
			</td>
		</tr>
		<tr>
	...
	{/if}
	<tr>
		<td>{'LBL_TOTAL_RECORDS_FAILED'|t:$MODULE}</td>
		<td width="10%">:</td>
		<td width="30%">{$IMPORT_RESULT.FAILED} / {$IMPORT_RESULT.TOTAL}
		{if $IMPORT_RESULT['FAILED'] neq '0'}
			&nbsp;&nbsp;<a class="cursorPointer" onclick="return window.open('index.php?module={$MODULE}&view=ListView&mode=getImportDetails&type=failed&start=1&foruser={$OWNER_ID}&forModule={$FOR_MODULE}','failed','width=700,height=650,resizable=no,scrollbars=yes,top=150,left=200');">{'LBL_DETAILS'|t:$MODULE}</a>
		{/if}
		</td>
	</tr>
</table>
