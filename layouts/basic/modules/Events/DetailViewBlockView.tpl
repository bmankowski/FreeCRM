							...
							{foreach key=KEY item=INVITIE from=$INVITIES_SELECTED}
								...
								<div>
									...
									{elseif $INVITIE['status'] == '2'}
										<span class="glyphicon glyphicon-minus-sign popoverTooltip" data-placement="top" data-content="{$STATUS_LABEL|t:$MODULE_NAME} {if $INVITIE['time']}({\App\Fields\DateTimeField::convertToUserFormat($INVITIE['time'])}){/if}" aria-hidden="true"></span>
									{else}
