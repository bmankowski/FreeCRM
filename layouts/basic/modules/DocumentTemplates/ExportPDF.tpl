...
{strip}
<!-- layouts/basic/modules/DocumentTemplates/ExportPDF.tpl -->
	<div class="modal-header">
		{if $ALL_RECORDS neq ''}
			<input type="hidden" name="all_records" id="all_records" value="{$ALL_RECORDS}" />
		{/if}
		<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
		<h3 id="massEditHeader" class="modal-title">{"LBL_GENERATE_PDF_FILE"|t:$QUALIFIED_MODULE}</h3>
		<br />
		<div class="panel panel-default">
			<div class="panel-heading"><strong>{"LBL_DOCUMENT_TEMPLATE"|t:$QUALIFIED_MODULE}</strong></div>
