...
<!-- layouts/basic/modules/ProjektyRekrutacyjne/RelatedListContents.tpl -->
	...
	<div class="listViewEntriesDiv contents-bottomscroll">
		<table class="table table-bordered listViewEntriesTable">
			...
			{if $RELATED_MODULE->isQuickSearchEnabled() && !($RELATED_LIST_SUPPRESS_QUICKSEARCH|default:false)}
				<tr>
					<td></td>
			...
			{foreach item=RELATED_RECORD from=$RELATED_RECORDS}
				<tr class="listViewEntries" data-id='{$RELATED_RECORD->getId()}'
					...
					>
					{assign var=COUNT value=0}
					<td class="{$WIDTHTYPE} noWrap" width="4%">
						<input type="checkbox" value="{$RELATED_RECORD->getId()}" class="listViewEntriesCheckBox" title="{"LBL_SELECT_SINGLE_ROW"|t}" />
					</td>
					...
					{/foreach}
					{if $SHOW_CREATOR_DETAIL}
						<td class="medium" data-field-type="rel_created_time" nowrap>{assign var="_datetime" value=\App\Fields\DateTimeField::getInstance($RELATED_RECORD->get('rel_created_time'))}{$_datetime->getDisplayDateTimeValue()}</td>
									...
									{foreach from=$INVENTORY_DATA item=ROWDATA}
										<tr>
											{if $INVENTORY_ROW['name']}
												{assign var="ROW_MODULE" value=\App\Records\Record::getType($INVENTORY_ROW['name'])}
											{/if}
											{foreach from=$INVENTORY_FIELDS item=FIELD key=NAME}
