...
<!-- layouts/basic/modules/OSSPasswords/DetailViewBlockView.tpl -->
				...
				{else}
				 ...
				 <div class="col-md-6 col-xs-12 fieldsLabelValue paddingLRZero">
					...
					<div class="fieldValue col-sm-7 col-xs-12 {$WIDTHTYPE}" id="{$MODULE}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} {assign var=COUNTER value=$COUNTER+1} {/if} {if $FIELD_MODEL->get('name') eq 'password'}onclick="showPasswordQuickEdit('{$smarty.get.record}');" {/if}>
						...
						{if $FIELD_MODEL->isEditable() eq 'true' && ($FIELD_MODEL->getFieldDataType()!=\App\Modules\Vtiger\Models\Field::REFERENCE_TYPE) && $IS_AJAX_ENABLED && $FIELD_MODEL->isAjaxEditable() eq 'true'}
    ...
    <div class="contentHeader row no-margin">
        <div class="pull-right">
            <button class="btn btn-success hide" id="copy-button" type="button" title="{"LBL_CopyToClipboardTitle"|t:$MODULE_NAME}"><span class="glyphicon glyphicon-copy"></span> {"LBL_CopyToClipboard"|t:$MODULE_NAME}</button>&nbsp;&nbsp;
            <button class="btn btn-warning" onclick="showDetailsPassword('{$smarty.get.record}');return false;" id="show-btn">{"LBL_ShowPassword"|t:$MODULE_NAME}</button>
        </div>
