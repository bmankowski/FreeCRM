...
-->*}
{strip}
<!-- layouts/basic/modules/Reports/step2.tpl -->
	<form class="form-horizontal recordEditView" id="report_step2" method="post" action="index.php">
		...
		<input type="hidden" name="selected_fields" id="seleted_fields" value='{\App\Modules\Vtiger\helpers\Util::toSafeHTML(\App\Json::encode($SELECTED_FIELDS))}' />
		...
		<input type="hidden" name="schdayoftheweek" value="{\App\Modules\Vtiger\helpers\Util::toSafeHTML(\App\Json::encode($REPORT_MODEL->get('schdayoftheweek')))}">
		<input type="hidden" name="schdayofthemonth" value="{\App\Modules\Vtiger\helpers\Util::toSafeHTML(\App\Json::encode($REPORT_MODEL->get('schdayofthemonth')))}">
		<input type="hidden" name="schannualdates" value="{\App\Modules\Vtiger\helpers\Util::toSafeHTML(\App\Json::encode($REPORT_MODEL->get('schannualdates')))}">
		<input type="hidden" name="recipients" value="{\App\Modules\Vtiger\helpers\Util::toSafeHTML(\App\Json::encode($REPORT_MODEL->get('recipients')))}">
		...
		<div class="well contentsBackground col-xs-12">
			<div class="block marginBottom10px col-xs-12 paddingLRZero">
				<div class="col-xs-12 marginBottom10px"><span class="redColor">*</span><strong>{"LBL_SELECT_COLUMNS"|t:$MODULE} ({"LBL_MAX"|t:$MODULE} {AppConfig::module('Reports','MAX_REPORT_COLUMN')})</strong></div>
				<div class="col-xs-12">
			...
			<div class="block col-xs-12 paddingLRZero paddingTop20">
				<div class="col-xs-12 paddingLRZero marginBottom10px">
					<span class="col-xs-6">
						<strong>{"LBL_GROUP_BY"|t:$MODULE}</strong>
					</span>
					<span class="col-xs-6">
						<strong>{"LBL_SORT_ORDER"|t:$MODULE}</strong>
					</span>
				</div>
			...
			<div class="paddingTop20 col-xs-12 block marginBottom10px ">
				<div class="marginBottom10px"><strong>{"LBL_CALCULATIONS"|t:$MODULE}</strong></div>
				<div class="">
					<table class="table tableRWD table-bordered CalculationFields" width="100%">
						<thead>
							<tr class="calculationHeaders">
								<th>{"LBL_COLUMNS"|t:$MODULE}</th>
								<th>{"LBL_SUM_VALUE"|t:$MODULE}</th>
								<th>{"LBL_AVERAGE"|t:$MODULE}</th>
								<th>{"LBL_LOWEST_VALUE"|t:$MODULE}</th>
								<th>{"LBL_HIGHEST_VALUE"|t:$MODULE}</th>
							</tr>
						</thead>
